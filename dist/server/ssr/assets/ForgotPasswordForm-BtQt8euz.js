...
function ForgotPasswordForm() {
	...
	async function handleSubmit(event) {
		...
		try {
			...
			if (!response.ok) setError(typeof data?.error === "string" ? data.error : "Não foi possível processar a solicitação. Tente novamente mais tarde.");
			else if (typeof data?.message === "string") setMessage(data.message);
			else setError("O servidor retornou uma resposta inesperada. Tente novamente.");
		} catch {
