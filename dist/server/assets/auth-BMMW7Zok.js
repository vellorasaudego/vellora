...
import { a as bcryptjs_default } from "./db-DG7c0MHC.js";
...
function getDraftSecret() {
	return "632b9b9f-9224-450a-8f94-feceb2c3ac43";
}
/**
...
async function requestSupabasePasswordReset(request, email, redirectTo, state) {
	const { error } = await createSupabaseRequestClient(request, state).auth.resetPasswordForEmail(email, { redirectTo });
	return error;
}
async function updateSupabasePassword(request, password, state) {
	...
	if (signOutError.error) console.error("[supabase-auth] Senha alterada, mas a revogação global falhou.", { error: signOutError.error.message });
	return {
		ok: true,
		error: null
	};
...
async function getLegacySession() {
	const { queryOne } = await import("./db-DV2EKxv6.js");
