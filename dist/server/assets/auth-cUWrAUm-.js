...
import { a as bcryptjs_default } from "./db-CnFqUhG4.js";
...
function getDraftSecret() {
	return "33572c7b-a477-485f-89ed-211cdf1d0882";
}
/**
...
//#endregion
//#region src/lib/auth-provider.ts
/**
...
*/
function resolveAuthProvider(value) {
...
//#endregion
//#region src/lib/supabase/config.ts
function validateSupabaseConfig(urlValue, keyValue) {
	const url = urlValue?.trim() || "";
	const publishableKey = keyValue?.trim() || "";
	if (!url || !publishableKey) throw new Error("Configure NEXT_PUBLIC_SUPABASE_URL e NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY antes de habilitar o Supabase Auth.");
	try {
...
function createSupabaseCookieState() {
	return {
		cookies: [],
		headers: {}
	};
...
function createSupabaseRequestClient(request, state) {
	const config = serverSupabaseConfig();
	return createServerClient(config.url, config.publishableKey, {
		auth: { flowType: "pkce" },
		cookies: {
...
*/
function createSupabaseStatelessClient() {
	const config = serverSupabaseConfig();
...
function applySupabaseCookieState(response, state) {
...
//#endregion
//#region src/lib/supabase/roles.ts
function mapSupabaseRole(value) {
...
//#endregion
//#region src/lib/supabase/auth.ts
var SUPABASE_GLOBAL_SIGN_OUT_TIMEOUT_MS = 5e3;
...
async function getSupabaseSessionFromClient(client) {
	const { data: { user }, error: userError } = await client.auth.getUser();
	if (userError || !user) return null;
	const { data: profile, error: profileError } = await client.from("profiles").select("name, role, active").eq("id", user.id).eq("active", true).maybeSingle();
	if (profileError) {
		console.error("[supabase-auth] Não foi possível validar o perfil ativo.", { error: profileError.message });
		return null;
	}
	return mapSupabaseSession(user, profile);
}
async function getSupabaseSession() {
	try {
		return getSupabaseSessionFromClient(await createSupabaseServerClient());
	} catch (error) {
		console.error("[supabase-auth] Não foi possível ler a sessão.", { error: error instanceof Error ? error.message : "Erro desconhecido" });
		return null;
	}
}
async function signInWithSupabase(request, email, password, state) {
	...
	if (error || !data.user) return {
		session: null,
		error: error || /* @__PURE__ */ new Error("Login sem usuário")
	};
	...
	if (!session) {
		...
		return {
			session: null,
			error: /* @__PURE__ */ new Error("Conta sem perfil ativo")
		};
	}
	return {
		session,
...
async function signOutWithSupabase(request, state) {
	const { error } = await createSupabaseRequestClient(request, state).auth.signOut({ scope: "local" });
	return error;
}
var SAFE_ORIGIN_PROTOCOLS = new Set(["http:", "https:"]);
function isLoopbackHostname(hostname) {
	const normalized = hostname.toLowerCase().replace(/^\[|\]$/g, "").replace(/\.$/, "");
	if (normalized === "localhost" || normalized === "::1") return true;
	const octets = normalized.split(".");
	return octets.length === 4 && octets[0] === "127" && octets.slice(1).every((octet) => /^\d{1,3}$/.test(octet) && Number(octet) <= 255);
}
function parseSafeOrigin(value, label) {
	let parsed;
	try {
	...
	if (!SAFE_ORIGIN_PROTOCOLS.has(parsed.protocol) || parsed.username || parsed.password || parsed.search || parsed.hash || parsed.pathname !== "" && parsed.pathname !== "/") throw new Error(`${label} deve apontar para uma origem HTTP(S) sem credenciais ou caminho.`);
	if (parsed.protocol === "http:" && !isLoopbackHostname(parsed.hostname)) throw new Error(`${label} deve usar HTTPS para hosts que não são de loopback.`);
	return new URL(parsed.origin);
}
function getSupabasePasswordResetRedirectUrl(requestUrl) {
	let requestOrigin;
	try {
	...
	const baseUrl = isLoopbackHostname(requestOrigin.hostname) ? parseSafeOrigin(requestOrigin.origin, "A origem da solicitação") : configuredAppUrl ? parseSafeOrigin(configuredAppUrl, "VELLORA_APP_URL") : parseSafeOrigin(requestOrigin.origin, "A origem da solicitação");
	return new URL("/auth/callback", baseUrl.origin).toString();
}
async function requestSupabasePasswordReset(email, redirectTo) {
	const { error } = await createSupabaseStatelessClient().auth.resetPasswordForEmail(email, { redirectTo });
	return error;
}
async function updateSupabasePassword(request, password, state) {
	const client = createSupabaseRequestClient(request, state);
	const { data: { user }, error: userError } = await client.auth.getUser();
	if (userError || !user) return {
		ok: false,
		error: userError || /* @__PURE__ */ new Error("Sessão ausente")
	};
	...
	try {
		const signOutResult = await withTimeout(client.auth.signOut({ scope: "global" }), SUPABASE_GLOBAL_SIGN_OUT_TIMEOUT_MS);
		if (signOutResult === null) console.warn("[supabase-auth] Senha alterada, mas a revogação global excedeu o tempo limite.", { timeoutMs: SUPABASE_GLOBAL_SIGN_OUT_TIMEOUT_MS });
		else if (signOutResult.error) console.error("[supabase-auth] Senha alterada, mas a revogação global falhou.", { error: signOutResult.error.message });
	} catch (error) {
		console.error("[supabase-auth] Senha alterada, mas a revogação global falhou.", { error: error instanceof Error ? error.message : "Erro desconhecido" });
	}
	return {
		ok: true,
		error: null
...
async function getLegacySession() {
	const { queryOne } = await import("./db-BVbT75nV.js");
...
//#endregion
export { VINEXT_CACHE_HEADER as $, markDynamicUsage as A, ACTION_REDIRECT_STATUS_HEADER as B, consumeDynamicUsage as C, getHeadersContext as D, getDraftModeCookieHeader as E, validateCookieName as F, MIDDLEWARE_NEXT_HEADER as G, ACTION_REVALIDATED_HEADER as H, buildRequestHeadersFromMiddlewareResponse as I, NEXTJS_ACTION_NOT_FOUND_HEADER as J, MIDDLEWARE_REWRITE_HEADER as K, encodeMiddlewareRequestHeaders as L, setHeadersContext as M, parseCookieHeader as N, headersContextFromRequest as O, serializeSetCookie as P, NEXT_URL_HEADER as Q, shouldKeepMiddlewareHeader as R, applyMiddlewareRequestHeaders as S, getAndClearPendingCookies as T, FLIGHT_HEADERS as U, ACTION_REDIRECT_TYPE_HEADER as V, INTERNAL_HEADERS as W, NEXT_ROUTER_SEGMENT_PREFETCH_HEADER as X, NEXT_ROUTER_PREFETCH_HEADER as Y, NEXT_ROUTER_STATE_TREE_HEADER as Z, createSupabaseServerClient as _, hashPassword as a, VINEXT_RSC_RENDER_MODE_HEADER as at, resolveAuthProvider as b, getSupabasePasswordResetRedirectUrl as c, createRequestContext as ct, signOutWithSupabase as d, runWithRequestContext as dt, VINEXT_INTERCEPTION_CONTEXT_HEADER as et, updateSupabasePassword as f, runWithUnifiedStateMutation as ft, createSupabaseRequestClient as g, __toESM as gt, createSupabaseCookieState as h, __exportAll as ht, getSession as i, VINEXT_PRERENDER_SECRET_HEADER as it, setHeadersAccessPhase as j, isDraftModeRequest as k, requestSupabasePasswordReset as l, getRequestContext as lt, applySupabaseCookieState as m, __commonJSMin as mt, createSessionToken as n, VINEXT_MW_CTX_HEADER as nt, roleHomePath as o, VINEXT_STATIC_FILE_HEADER as ot, mapSupabaseSession as p, getOrCreateAls as pt, MIDDLEWARE_SET_COOKIE_HEADER as q, getAuthProvider as r, VINEXT_PARAMS_HEADER as rt, verifyPassword as s, VINEXT_TIMING_HEADER as st, SESSION_COOKIE_NAME as t, VINEXT_MOUNTED_SLOTS_HEADER as tt, signInWithSupabase as u, isInsideUnifiedScope as ut, validateSupabaseConfig as v, consumeInvalidDynamicUsageError as w, jwtVerify as x, createClient as y, ACTION_REDIRECT_HEADER as z };
