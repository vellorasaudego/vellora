...
var require_react_server_dom_webpack_server_edge_production = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	* LICENSE file in the root directory of this source tree.
	*/
	var ReactDOM = require_react_dom_react_server(), React = require_react_react_server();
	function noop() {}
...
async function requireRole(...roles) {
	...
	if (!roles.includes(session.role)) return { error: NextResponse.json({ error: "Sem permissão." }, { status: 403 }) };
	return { session };
}
//#endregion
...
//#endregion
//#region src/lib/record-utils.ts
var DAILY_RECORD_AUDIT_FIELDS = [
	"record_date",
	"record_time",
	"bp_systolic",
	"bp_diastolic",
...
*/
function snapshotDailyRecord(record) {
	const snapshot = {};
	for (const field of DAILY_RECORD_AUDIT_FIELDS) snapshot[field] = field === "photo_data" ? Boolean(record[field]) : record[field] ?? null;
	return snapshot;
}
function diffDailyRecord(before, after) {
	const beforeSnapshot = snapshotDailyRecord(before);
	const afterSnapshot = snapshotDailyRecord(after);
	return DAILY_RECORD_AUDIT_FIELDS.filter((field) => JSON.stringify(beforeSnapshot[field]) !== JSON.stringify(afterSnapshot[field]));
}
function saoPauloDateTime(date = /* @__PURE__ */ new Date()) {
...
async function updatePatient(id, fields) {
	...
	const values = [];
	let i = 1;
	for (const key of allowed) if (key in fields) {
		sets.push(`${key} = $${i}`);
...
async function updateProfessionalApplicationStatus(id, status, reviewedBy) {
	...
	await query("UPDATE professional_applications SET status = $1, reviewed_at = now(), reviewed_by = $2 WHERE id = $3", [
		status,
		reviewedBy,
		id
	]);
...
async function contractKeysOwnedBy(column, ownerId) {
	return (await query(`SELECT storage_key FROM contract_documents WHERE ${column} = $1`, [ownerId])).map((contract) => contract.storage_key);
}
async function cleanupStoredContracts(keys) {
	try {
		await deleteStoredFiles(keys);
	} catch (error) {
		console.error("[contracts] Metadados removidos, mas alguns arquivos não puderam ser limpos.", { error: error instanceof Error ? error.message : "Erro desconhecido" });
	}
}
async function deleteCaregiverProfile(id) {
	...
	if (profile.user_id) keys.push(...await contractKeysOwnedBy("caregiver_user_id", profile.user_id));
	const commands = [{
		text: "DELETE FROM contract_documents WHERE caregiver_profile_id = $1",
		params: [id]
	}];
	if (profile.user_id) commands.push({
		text: "DELETE FROM contract_documents WHERE caregiver_user_id = $1",
		params: [profile.user_id]
	}, {
		text: "DELETE FROM caregiver_assignments WHERE caregiver_user_id = $1",
		params: [profile.user_id]
	});
	commands.push({
		text: "DELETE FROM caregiver_profiles WHERE id = $1",
		params: [id]
	});
	if (profile.user_id) commands.push({
		text: `UPDATE users
	...
	await executeBatch([
		...
		},
		{
			text: `UPDATE users
             ...
             WHERE id = $3 AND role = 'cuidador' AND deleted_at IS NULL`,
			params: [
				`removido-${id}@anonimo.vellora.invalid`,
				hashPassword(randomUUID()),
				id
			]
		},
...
async function deleteFamilyUser(id) {
	...
	await executeBatch([
		{
			text: "UPDATE patients SET family_user_id = NULL WHERE family_user_id = $1",
			params: [id]
		},
		{
			text: "DELETE FROM contract_documents WHERE family_user_id = $1",
			params: [id]
		},
		{
			text: `UPDATE users
...
function contractOwnerWhere(ownerType) {
	if (ownerType === "family") return "family_user_id";
	if (ownerType === "caregiver_profile") return "caregiver_profile_id";
	return "caregiver_user_id";
}
async function listContractDocuments(ownerType, ownerId) {
	...
	return query(`SELECT id, family_user_id, caregiver_profile_id, caregiver_user_id,
     ...
     WHERE ${contractOwnerWhere(ownerType)} = $1
...
async function createContractDocument(input) {
	...
	await putStoredFile(storageKey, input.fileData, input.mimeType);
	try {
		await query(`INSERT INTO contract_documents
	...
	return await queryOne(`SELECT id, family_user_id, caregiver_profile_id, caregiver_user_id,
            file_name, mime_type, file_size, uploaded_by, created_at
     FROM contract_documents WHERE id = $1`, [id]);
}
...
async function POST$12(req, { params }) {
	...
	if (!/^\S+@\S+\.\S+$/.test(email)) return NextResponse.json({ error: "Informe um e-mail de acesso válido." }, { status: 400 });
	if (password.length < 12) return NextResponse.json({ error: "A senha provisória deve ter pelo menos 12 caracteres." }, { status: 400 });
	const profile = await getCaregiverProfile(id);
	if (!profile) return NextResponse.json({ error: "Profissional não encontrado no banco de cuidadores." }, { status: 404 });
	if (profile.user_id) return NextResponse.json({ error: "Este profissional já possui acesso ao painel." }, { status: 409 });
	if (profile.account_status !== "aguardando_acesso") return NextResponse.json({ error: "Este perfil não está disponível para criação de acesso." }, { status: 409 });
	if (await getUserByEmail(email)) return NextResponse.json({ error: "Já existe uma conta com esse e-mail." }, { status: 409 });
	try {
		const user = await createCaregiverAccess({
			profileId: id,
			email,
			password
		});
		...
		return NextResponse.json({ error: "Não foi possível criar o acesso. Verifique o e-mail e tente novamente." }, { status: 400 });
	}
}
...
var metadata$4 = {
	...
	description: "Cuidado domiciliar planejado para cada família, com cuidadores e profissionais de enfermagem, gestão de escalas e acompanhamento digital em Goiânia e região.",
	keywords: [
		"home care Goiânia",
...
//#endregion
//#region src/lib/vitals.ts
function statusBP(systolic, diastolic) {
...
function statusGlucose(glucose) {
...
//#endregion
//#region src/components/RecordCard.tsx
var MOOD_LABEL = {
	bem: "😊 Bem",
	neutro: "😐 Neutro",
...
function formatDate$1(dateStr) {
	return (/* @__PURE__ */ new Date(dateStr + "T00:00:00")).toLocaleDateString("pt-BR", {
		weekday: "long",
		day: "2-digit",
		month: "long",
		year: "numeric"
...
function RecordCard({ record, caregiverName, editHref }) {
	const overall = worstStatus(statusBP(record.bp_systolic, record.bp_diastolic), statusHeartRate(record.heart_rate), statusTemperature(record.temperature), statusSpo2(record.spo2), statusGlucose(record.glucose));
		...
		children: [
			/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
				...
				children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", { children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", {
					className: "text-sm font-semibold text-[var(--foreground)] capitalize",
					children: formatDate$1(record.record_date)
				}), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("p", {
					className: "mt-1 flex flex-wrap items-center gap-x-2 text-xs text-[var(--muted-2)]",
					children: [
						/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("span", { children: record.record_time ? `Sinais vitais medidos às ${record.record_time}` : "Horário da medição não informado" }),
						caregiverName && /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)(import_jsx_runtime_react_server.Fragment, { children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("span", {
							"aria-hidden": "true",
							children: "•"
						}), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("span", { children: ["Registrado por ", caregiverName] })] }),
						record.updated_at && record.updated_at !== record.created_at && /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)(import_jsx_runtime_react_server.Fragment, { children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("span", {
							"aria-hidden": "true",
							children: "•"
						}), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("span", { children: "Atualizado posteriormente" })] })
					]
				})] }), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
					className: "flex items-center gap-2",
					children: [record.incident === 1 && /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(StatusBadge, {
						status: "critical",
						label: "Intercorrência"
					}), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(StatusBadge, { status: overall })]
				})]
			}),
			/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
				className: "grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4 mb-4 text-sm",
				children: [
					/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(VitalCell, {
						label: "Pressão",
						value: record.bp_systolic && record.bp_diastolic ? `${record.bp_systolic}/${record.bp_diastolic}` : "—",
						unit: "mmHg"
					}),
					/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(VitalCell, {
						label: "Freq. cardíaca",
			...
			/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("dl", {
				className: "grid sm:grid-cols-2 gap-x-6 gap-y-3 text-sm border-t border-[var(--border)] pt-4",
				children: [
					record.medications && /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", { children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("dt", {
					...
					record.feeding && /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", { children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("dt", {
						className: "text-[var(--muted-2)]",
					...
					record.hygiene && /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", { children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("dt", {
						className: "text-[var(--muted-2)]",
						children: "Higiene"
					}), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("dd", {
						className: "text-[var(--foreground)]",
						children: record.hygiene
					})] }),
					record.mobility && /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", { children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("dt", {
						className: "text-[var(--muted-2)]",
						children: "Mobilidade / atividades"
					}), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("dd", {
						className: "text-[var(--foreground)]",
						children: record.mobility
					})] }),
					record.mood && /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", { children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("dt", {
						className: "text-[var(--muted-2)]",
						children: "Humor"
					}), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("dd", {
						className: "text-[var(--foreground)]",
						children: MOOD_LABEL[record.mood] || record.mood
					})] }),
					record.pain_level != null && /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", { children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("dt", {
						className: "text-[var(--muted-2)]",
						children: "Nível de dor"
					}), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("dd", {
						className: "text-[var(--foreground)]",
						children: [record.pain_level, "/10"]
					})] })
				]
			}),
			record.notes && /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("p", {
				className: "mt-4 text-sm text-[var(--muted)] border-t border-[var(--border)] pt-4",
				children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("span", {
					className: "text-[var(--muted-2)]",
					children: "Observações: "
				}), record.notes]
			}),
			record.incident === 1 && record.incident_description && /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
				className: "mt-4 rounded-xl bg-[var(--status-critical-bg)] p-4 text-sm text-[var(--status-critical)]",
				children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", {
					className: "font-medium mb-1",
					children: "Intercorrência relatada"
				}), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", { children: record.incident_description })]
			}),
			record.photo_data && /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("img", {
			...
			editHref && /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("div", {
...
function VitalCell({ label, value, unit }) {
	return /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
		className: "rounded-xl bg-black/[0.02] p-3",
		children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", {
			...
			children: [
				...
				/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("span", {
					className: "text-xs font-normal text-[var(--muted-2)]",
					children: unit
				})
			]
		})]
...
//#endregion
//#region src/app/cuidador/paciente/[id]/historico/page.tsx
var page_exports$20 = /* @__PURE__ */ __exportAll({ default: () => HistoricoPage });
async function HistoricoPage({ params }) {
	const { id } = await params;
	const session = await getSession();
	const patient = await getPatient(id);
	if (!patient || !session || !await isCaregiverAssignedToPatient(session.userId, id)) notFound();
	...
	return /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
		...
		children: [
			...
			/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("h2", {
				...
				children: ["Histórico — ", patient.name]
			}),
			/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
				className: "mt-6 space-y-4",
				children: [records.length === 0 && /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", {
					className: "text-sm text-[var(--muted-2)]",
					children: "Nenhum registro ainda."
				}), records.map((r) => /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(RecordCard, {
					record: r,
					caregiverName: namesMap[r.caregiver_user_id] || "Cuidador",
					editHref: r.caregiver_user_id === session.userId ? `/cuidador/paciente/${patient.id}/registro?recordId=${r.id}` : void 0
				}, r.id))]
			})
		]
	});
		...
		children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("span", {
			...
			children: /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("span", {
				...
				children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("span", {
					...
					className: "-ml-0.5 text-sm text-[#a8eee2]",
					children: "+"
				})]
			})
		}), !compact && /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("span", {
			className: "leading-none",
			children: /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("span", {
				className: "block text-[1.05rem] font-bold tracking-[-0.03em] text-[var(--foreground)]",
				children: "Vellora Saúde"
			})
		})]
	});
}
//#endregion
//#region src/components/DashboardShell.tsx
function DashboardShell({ title, userName, roleLabel, navItems, children }) {
	return /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
		className: "flex min-h-screen bg-[#f4f7f5]",
		children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("aside", {
			className: "hidden w-72 flex-col border-r border-[var(--border)] bg-[var(--surface)] md:flex",
			children: [
				/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("div", {
					className: "flex h-[5.25rem] items-center border-b border-[var(--border)] px-6",
					children: /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(Brand, {})
				}),
				/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("nav", {
					"aria-label": "Navegação da área exclusiva",
					className: "flex-1 space-y-1.5 px-4 py-6",
					children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", {
						className: "px-3 pb-2 text-[0.64rem] font-bold uppercase tracking-[0.16em] text-[var(--muted-2)]",
						children: "Menu"
					}), navItems.map((item) => /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)(link_default, {
						href: item.href,
						...
						children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("span", {
							className: "flex h-8 w-8 items-center justify-center rounded-lg bg-[var(--surface-soft)] text-sm",
							"aria-hidden": "true",
							children: item.icon
						}), item.label]
					}, item.href))]
				...
				/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
					className: "border-t border-[var(--border)] px-6 py-5 text-xs leading-5 text-[var(--muted-2)]",
					children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", {
						className: "font-semibold text-[var(--brand-dark)]",
						children: "Ambiente protegido"
					}), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", { children: "Acesso restrito a usuários autorizados." })]
				})
			]
		}), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
			className: "flex-1 flex flex-col min-w-0",
			children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("header", {
				className: "sticky top-0 z-30 flex min-h-[5.25rem] items-center justify-between gap-4 border-b border-[var(--border)] bg-[var(--surface)]/95 px-4 backdrop-blur md:px-8",
				children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
					className: "flex min-w-0 items-center gap-3",
					children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("details", {
						className: "relative md:hidden",
						children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("summary", {
							className: "flex h-10 w-10 cursor-pointer list-none items-center justify-center rounded-xl border border-[var(--border)] text-[var(--brand-dark)]",
							...
							className: "absolute left-0 top-12 z-50 w-64 space-y-1 rounded-2xl border border-[var(--border)] bg-white p-3 shadow-xl",
							"aria-label": "Menu móvel",
							children: navItems.map((item) => /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)(link_default, {
								href: item.href,
								className: "flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-semibold text-[var(--muted)] hover:bg-[var(--brand-light)]",
								children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("span", {
									"aria-hidden": "true",
									children: item.icon
								}), item.label]
						...
						className: "min-w-0",
						children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", {
							className: "text-[0.64rem] font-bold uppercase tracking-[0.14em] text-[var(--accent-dark)]",
							children: roleLabel
					...
					}), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(LogoutButton, {})]
				})]
			}), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("main", {
				className: "flex-1 overflow-x-hidden p-4 sm:p-6 md:p-8 lg:p-10",
				children
			})]
		})]
	});
...
//#endregion
//#region src/app/cuidador/layout.tsx
var layout_exports$2 = /* @__PURE__ */ __exportAll({ default: () => CuidadorLayout });
async function CuidadorLayout({ children }) {
	return /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(DashboardShell, {
		title: "Área do cuidador",
		userName: (await getSession())?.name || "",
		roleLabel: "Cuidador",
		navItems: [{
			href: "/cuidador",
...
var DailyRecordForm = /* @__PURE__ */ registerClientReference(() => {
	throw new Error("Unexpectedly client reference export 'DailyRecordForm' is called on server");
}, "108487685012", "DailyRecordForm");
//#endregion
//#region src/app/cuidador/paciente/[id]/registro/page.tsx
var page_exports$19 = /* @__PURE__ */ __exportAll({ default: () => RegistroDiarioPage });
async function RegistroDiarioPage({ params, searchParams }) {
	const { id } = await params;
	...
	const requestedRecordId = typeof query.recordId === "string" ? query.recordId : void 0;
	const requestedRecord = requestedRecordId ? await getRecord(requestedRecordId) : void 0;
	if (requestedRecordId && (!requestedRecord || requestedRecord.patient_id !== id || requestedRecord.caregiver_user_id !== session.userId)) notFound();
	...
	return /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
		...
		children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(link_default, {
			...
			className: "mt-4 rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-6 md:p-8",
			children: [
				/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("h2", {
					className: "text-lg font-semibold text-[var(--foreground)]",
					children: [
						existingToday ? "Editar registro" : "Registro diário",
						" — ",
						patient.name
					]
				}),
				/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", {
					...
					children: existingToday ? "Atualize as informações salvas. O histórico manterá o registro da alteração." : "Preencha as informações do atendimento de hoje. Os campos de sinais vitais são opcionais quando não aferidos, mas recomendamos preenchê-los sempre que possível."
				}),
				/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("div", {
					className: "mt-6",
					children: /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(DailyRecordForm, {
...
//#endregion
//#region src/lib/api-error.ts
function apiError(error, context, publicMessage) {
	console.error(`[${context}] ${publicMessage}`, { error: error instanceof Error ? error.message : "Erro desconhecido" });
...
async function DELETE$4(_req, { params }) {
	...
	try {
		...
		return apiError(error, "api/admin/contracts", "Não foi possível excluir o contrato.");
	}
}
//#endregion
...
async function PATCH$3(req, { params }) {
	const guard = await requireRole("admin");
	if ("error" in guard) return guard.error;
	const { id } = await params;
	...
	if (![
		...
		"convertido",
		"recusado"
	].includes(status)) return NextResponse.json({ error: "Status inválido." }, { status: 400 });
	try {
...
async function DELETE$2(_req, { params }) {
	const guard = await requireRole("admin");
	if ("error" in guard) return guard.error;
	const { id } = await params;
	try {
...
async function PATCH$2(req, { params }) {
	const guard = await requireRole("admin");
	if ("error" in guard) return guard.error;
	const { id } = await params;
	const body = await req.json().catch(() => null);
	if (!body) return NextResponse.json({ error: "Corpo inválido." }, { status: 400 });
	let familyUserId = body.family_user_id;
	if (!familyUserId && body.new_family_name && body.new_family_email && body.new_family_password) {
		if (body.new_family_password.length < 12) return NextResponse.json({ error: "A senha provisória deve ter pelo menos 12 caracteres." }, { status: 400 });
		if (await getUserByEmail(body.new_family_email.toLowerCase().trim())) return NextResponse.json({ error: "Já existe um usuário com esse e-mail de família." }, { status: 400 });
		familyUserId = (await createUser({
...
async function DELETE$1(_req, { params }) {
	const guard = await requireRole("admin");
	if ("error" in guard) return guard.error;
	const { id } = await params;
...
async function PATCH$1(req, { params }) {
	const guard = await requireRole("admin");
	if ("error" in guard) return guard.error;
	const { id } = await params;
	const status = (await req.json().catch(() => null))?.status;
...
//#endregion
//#region src/components/LineIcon.tsx
function LineIcon({ name, className = "h-5 w-5" }) {
	return /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("svg", {
		...
		children: (() => {
			switch (name) {
				case "activity": return /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("path", { d: "M3 12h4l2.2-6 4.1 12 2.1-6H21" });
				case "arrow": return /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)(import_jsx_runtime_react_server.Fragment, { children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("path", { d: "M5 12h14" }), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("path", { d: "m13 6 6 6-6 6" })] });
				case "care": return /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)(import_jsx_runtime_react_server.Fragment, { children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("path", { d: "M20.8 4.7a5.5 5.5 0 0 0-7.8 0L12 5.8l-1.1-1.1a5.5 5.5 0 0 0-7.8 7.8L12 21l8.8-8.5a5.5 5.5 0 0 0 0-7.8Z" }), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("path", { d: "M8 12h2l1-2 2 4 1-2h2" })] });
				case "check": return /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)(import_jsx_runtime_react_server.Fragment, { children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("circle", {
					cx: "12",
					cy: "12",
					r: "9"
				}), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("path", { d: "m8 12 2.5 2.5L16 9" })] });
				case "clock": return /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)(import_jsx_runtime_react_server.Fragment, { children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("circle", {
					cx: "12",
					cy: "12",
					r: "9"
				}), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("path", { d: "M12 7v5l3 2" })] });
				case "home": return /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)(import_jsx_runtime_react_server.Fragment, { children: [
					/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("path", { d: "m3 11 9-8 9 8" }),
					/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("path", { d: "M5 10v10h14V10" }),
					/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("path", { d: "M9 20v-6h6v6" })
				] });
				case "medical": return /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)(import_jsx_runtime_react_server.Fragment, { children: [
					/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("path", { d: "M6 3v5a6 6 0 0 0 12 0V3" }),
					/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("path", { d: "M3 3h6" }),
					/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("path", { d: "M15 3h6" }),
					/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("path", { d: "M12 14v2a4 4 0 0 0 4 4h1" }),
				...
				case "person": return /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)(import_jsx_runtime_react_server.Fragment, { children: [
					/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("circle", {
						cx: "9",
						cy: "8",
						r: "4"
					}),
					/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("path", { d: "M2 21v-2a7 7 0 0 1 14 0v2" }),
					/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("path", { d: "m17 11 2 2 3-4" })
				] });
				case "shield": return /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)(import_jsx_runtime_react_server.Fragment, { children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("path", { d: "M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10Z" }), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("path", { d: "m9 12 2 2 4-4" })] });
			}
		})()
	});
}
...
function WhatsAppButton() {
	return /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("a", {
		...
		children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("svg", {
			"aria-hidden": "true",
			viewBox: "0 0 24 24",
			className: "h-5 w-5",
			fill: "none",
			...
			strokeLinecap: "round",
			strokeLinejoin: "round",
			children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("path", { d: "M20 11.5a8 8 0 0 1-11.8 7l-4.2 1.1 1.1-4A8 8 0 1 1 20 11.5Z" }), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("path", { d: "M8.7 8.2c.3 2.8 2.4 5 5.1 5.7l1.1-1.2c.2-.2.5-.3.8-.1l1.6.7c.3.1.4.4.4.7-.2 1.4-1.4 2.3-2.8 2.2-4.7-.4-8.4-4.1-8.8-8.8-.1-1.4.8-2.6 2.2-2.8.3 0 .6.1.7.4l.7 1.6c.1.3.1.6-.1.8L8.7 8.2Z" })]
		}), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("span", { children: "Fale conosco" })]
	});
}
//#endregion
//#region src/components/PublicFooter.tsx
function PublicFooter() {
	return /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("footer", {
		...
		children: [
			/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
				className: "container-page grid gap-10 py-12 md:grid-cols-[1.4fr_0.8fr_0.8fr]",
				children: [
					/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", { children: [
						/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("div", {
							className: "inline-flex rounded-lg bg-white px-3 py-2.5",
							children: /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(Brand, {})
						}),
						/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", {
							className: "mt-5 max-w-md text-sm leading-6 text-white/68",
							children: "Gestão e assistência domiciliar com cuidado humano, organização profissional e informação clara para a família."
						}),
						/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", {
							className: "mt-4 text-sm font-medium text-[#a9e8dc]",
							children: "Atendimento em Goiânia e região"
						})
					] }),
					/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", { children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", {
						...
						className: "mt-4 flex flex-col gap-2.5 text-sm text-white/72",
						children: [
							/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(link_default, {
								href: "/#servicos",
					...
					/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", { children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", {
						...
						children: [
							/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(link_default, {
								href: "/login",
								className: "hover:text-white",
								children: "Painel da família"
							}),
							/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(link_default, {
								href: "/login",
								className: "hover:text-white",
								children: "Painel do profissional"
							}),
							/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(link_default, {
								href: "/solicitar-cuidado",
								className: "hover:text-white",
								children: "Solicitar cuidado"
							})
						]
					})] })
				]
...
//#endregion
//#region src/components/PublicNav.tsx
function PublicNav() {
	return /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("header", {
		className: "sticky top-0 z-40 border-b border-[var(--border)] bg-white/95 backdrop-blur-lg",
		children: /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
			className: "container-page flex h-[4.5rem] items-center justify-between gap-5",
			children: [
				/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(Brand, {}),
				/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("nav", {
					"aria-label": "Navegação principal",
					className: "hidden items-center gap-9 text-sm font-medium text-[var(--muted)] xl:flex",
					children: [
						/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(link_default, {
							href: "/#servicos",
...
var page_exports$18 = /* @__PURE__ */ __exportAll({ default: () => HomePage });
var SERVICES = [
	{
...
var CARE_ROUTINE = [
	...
	{
		label: "Higiene",
		value: "Cuidados realizados",
		icon: "care"
	},
	{
		label: "Mobilidade",
		value: "Caminhada assistida",
		icon: "person"
	}
];
function HomePage() {
	return /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)(import_jsx_runtime_react_server.Fragment, { children: [
		/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(PublicNav, {}),
		/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("main", {
			className: "flex-1",
			children: [
				/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("section", {
					className: "home-hero border-b border-[var(--border)] bg-white",
					children: [
						/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("span", {
						...
						/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
							className: "container-page relative z-[1] grid min-h-[680px] items-center gap-14 py-16 lg:grid-cols-[1.03fr_0.97fr] lg:py-20",
							children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", { children: [
								/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("span", {
									className: "hero-kicker inline-flex items-center gap-2 rounded-full px-4 py-2 text-sm font-semibold text-[var(--brand-dark)]",
									children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(LineIcon, {
										name: "shield",
										className: "h-4 w-4"
									}), "Cuidado profissional em casa"]
								}),
								/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("h1", {
									className: "font-display mt-7 max-w-[680px] text-[clamp(3rem,5.7vw,5.25rem)] leading-[0.98] text-[var(--foreground)]",
									children: ["Cuidar de quem você ama, ", /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("span", {
										className: "text-[var(--brand)]",
										children: "com quem cuida por profissão."
									})]
								}),
								/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", {
									className: "mt-7 max-w-2xl text-base leading-8 text-[var(--muted)] sm:text-lg",
									children: "A Vellora Saúde seleciona e gerencia cuidadores e profissionais de saúde para atender seu familiar em casa, com supervisão de enfermagem e acompanhamento contínuo."
								}),
								/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
									...
									children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("a", {
										...
										children: ["Solicitar cuidado", /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(LineIcon, {
											name: "arrow",
											className: "h-4 w-4"
										})]
								...
								/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
									className: "mt-9 flex flex-wrap gap-x-8 gap-y-3 text-sm text-[var(--muted)]",
									children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("span", {
								...
								className: "hero-care-card rounded-3xl border border-[#bdeee5] bg-white/94 p-6 shadow-[0_24px_60px_rgba(27,82,75,0.16)] backdrop-blur-[5px] sm:p-8 lg:ml-6",
								children: [
									/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
										className: "flex items-center gap-4",
										children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("span", {
											className: "flex h-14 w-14 shrink-0 items-center justify-center rounded-xl bg-[var(--brand-dark)] text-white",
											children: /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(LineIcon, {
												name: "care",
												className: "h-7 w-7"
											})
											...
											children: "Plano de cuidado sob medida"
										})] })]
									}),
									/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("div", {
										className: "mt-7 grid gap-3 sm:grid-cols-2",
										children: HERO_SERVICES.map((service) => /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
											className: "flex min-h-14 items-center gap-3 rounded-xl bg-[var(--brand-light)] px-4 text-sm font-medium text-[var(--foreground)]",
											children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(LineIcon, {
												name: service.icon,
												className: "h-5 w-5 text-[var(--brand-dark)]"
											}), service.label]
										}, service.label))
									}),
									/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
										className: "mt-7 flex flex-col gap-3 rounded-xl bg-[var(--surface-soft)] px-5 py-4 text-sm text-[var(--muted)] sm:flex-row sm:items-center sm:justify-between",
										children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("span", {
											className: "inline-flex items-center gap-2 font-semibold text-[var(--brand-dark)]",
											children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(LineIcon, {
												name: "home",
												className: "h-4 w-4"
											}), "Goiânia e região"]
										}), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("span", { children: "Profissional conforme cada necessidade" })]
									}),
									/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
										className: "mt-5 flex items-center gap-3 border-t border-[var(--border)] pt-5 text-xs text-[var(--muted)]",
										children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("span", {
											className: "flex -space-x-2",
											"aria-hidden": "true",
											children: [
												/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("span", { className: "h-8 w-8 rounded-full border-2 border-white bg-[#d9f1ec]" }),
												/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("span", { className: "h-8 w-8 rounded-full border-2 border-white bg-[#f8dbc2]" }),
												/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("span", {
													className: "flex h-8 w-8 items-center justify-center rounded-full border-2 border-white bg-[var(--brand-dark)] text-[10px] font-bold text-white",
													children: "V+"
												})
											]
										}), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("span", { children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("strong", {
											className: "text-[var(--foreground)]",
											children: "Cuidado coordenado"
										}), " para manter a família bem informada."] })]
									})
								]
							})]
						})
				...
				/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("section", {
					"aria-label": "Compromissos da Vellora Saúde",
					className: "trust-ribbon-wrap",
					children: /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("div", {
						className: "container-page",
						children: /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("div", {
							className: "trust-ribbon grid gap-1 sm:grid-cols-3",
							children: TRUST_POINTS.map((item) => /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("article", {
				...
				/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("section", {
					id: "servicos",
					className: "services-section py-20 sm:py-28",
					children: /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
						...
						children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
							...
							children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", { children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("span", {
								...
								children: "Cuidado completo, no conforto de casa"
							})] }), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", {
								className: "max-w-md text-base leading-7 text-[var(--muted)] lg:text-right",
								children: "Cada plano é organizado a partir da rotina, das necessidades e das preferências da família."
							})]
						}), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("div", {
							className: "mt-12 grid gap-5 sm:grid-cols-2 lg:grid-cols-3",
							children: SERVICES.map((service, index) => /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("article", {
								...
								children: [
									...
									/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("h3", {
										className: "mt-6 text-lg font-bold tracking-[-0.02em] text-[var(--foreground)]",
									...
									/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("a", {
										href: "/solicitar-cuidado",
										className: "mt-6 inline-flex items-center gap-2 text-xs font-bold text-[var(--brand-dark)]",
										children: ["Conversar sobre este cuidado", /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(LineIcon, {
											name: "arrow",
				...
				/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("section", {
					id: "como-funciona",
					className: "process-section py-20 sm:py-28",
					children: /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
						className: "container-page",
						children: [
							/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("span", {
							...
							/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("h2", {
								...
								children: "Do primeiro contato ao cuidado contínuo"
							}),
							/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("div", {
								className: "process-grid mt-12 grid gap-5 sm:grid-cols-2 lg:grid-cols-4",
								children: STEPS.map((step) => /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("article", {
									...
									children: [
										...
										/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("h3", {
											className: "mt-6 text-base font-bold text-[var(--foreground)]",
											children: step.title
										}),
										/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", {
											className: "mt-3 text-sm leading-6 text-[var(--muted)]",
				...
				/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("section", {
					...
					children: /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
						className: "container-page grid items-stretch gap-10 lg:grid-cols-[0.86fr_1.14fr] lg:gap-16",
						children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("div", {
							className: "why-photo-panel",
							children: /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
								className: "why-photo-content",
								children: [
									/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("span", {
										className: "inline-flex rounded-full bg-white/16 px-3 py-1.5 text-xs font-bold text-white backdrop-blur",
										children: "Cuidado com presença"
									}),
									/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", {
										className: "mt-4 max-w-sm text-2xl font-bold leading-tight tracking-[-0.04em] text-white sm:text-3xl",
										children: "Organização para a família. Acolhimento para quem recebe o cuidado."
									}),
									/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
										className: "mt-6 flex items-center gap-3 text-sm text-white/82",
										children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("span", {
											className: "flex h-10 w-10 items-center justify-center rounded-full bg-white text-[var(--brand-dark)]",
											children: /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(LineIcon, {
												name: "care",
												className: "h-5 w-5"
											})
										}), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("span", { children: "Um plano pensado para a rotina real da casa." })]
									})
								]
							})
						}), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
							...
							children: [
								/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("span", {
									className: "eyebrow",
									children: "Por que a Vellora Saúde"
								}),
								/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("h2", {
									className: "section-title mt-4 max-w-lg",
									children: "Profissionais que você pode confiar"
								}),
								/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", {
									className: "mt-5 max-w-lg text-base leading-7 text-[var(--muted)]",
									children: "Selecionamos profissionais, conferimos informações e acompanhamos o atendimento. A família recebe orientação e o paciente mantém sua rotina com mais segurança."
								}),
								/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("div", {
									className: "mt-8 grid gap-4 sm:grid-cols-2",
									children: DIFFERENTIALS.map((item) => /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("article", {
										className: "differential-card",
										children: [
											/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("span", {
												className: "flex h-10 w-10 items-center justify-center rounded-xl bg-[var(--brand-dark)] text-white shadow-[0_8px_18px_rgba(28,87,79,0.18)]",
												children: /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(LineIcon, {
													name: item.icon,
													className: "h-4 w-4"
												})
											...
											/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("h3", {
												className: "mt-4 text-sm font-bold text-[var(--foreground)]",
												children: item.title
											}),
											/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", {
				...
				/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("section", {
					id: "acompanhamento",
					className: "portal-section py-20 sm:py-28",
					children: /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
						className: "container-page grid items-center gap-12 lg:grid-cols-[0.9fr_1.1fr] lg:gap-20",
						children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", { children: [
							/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("span", {
							...
							/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("h2", {
								...
								children: "Acompanhe a rotina de cuidado"
							}),
							/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", {
								className: "mt-5 max-w-lg text-base leading-7 text-white/72",
								children: "Usuários autorizados consultam os registros recentes e o histórico do atendimento em uma área exclusiva."
							}),
							/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
								className: "mt-6 rounded-2xl border border-white/14 bg-white/9 p-5 backdrop-blur-sm",
								children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("h3", {
									className: "text-sm font-bold text-white",
									children: "O que é o resumo do cuidado?"
								}), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", {
									className: "mt-2 text-sm leading-6 text-white/68",
									children: "É uma leitura rápida dos registros do período, como cuidados realizados, sinais vitais, observações da equipe e horário da última atualização."
								})]
							}),
							/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("a", {
								href: "/login",
								className: "mt-7 inline-flex items-center gap-2 rounded-xl bg-white px-5 py-3 text-sm font-bold text-[var(--brand-deep)] shadow-lg hover:-translate-y-0.5",
								children: ["Acessar o painel", /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(LineIcon, {
									name: "arrow",
									className: "h-4 w-4"
								})]
							})
						] }), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
							className: "portal-summary-card overflow-hidden rounded-3xl border border-white/50 bg-white shadow-[0_28px_70px_rgba(6,29,26,0.28)]",
							children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
								className: "flex flex-col gap-3 border-b border-[var(--border)] px-6 py-5 sm:flex-row sm:items-start sm:justify-between",
								children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", { children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("h3", {
									...
									children: "Resumo do cuidado"
								}), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", {
									className: "mt-1 text-sm text-[var(--muted)]",
									children: "Hoje · atualização às 14h20"
								})] }), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("span", {
									className: "inline-flex w-fit items-center gap-2 rounded-full bg-[var(--brand-light)] px-3 py-1 text-xs font-semibold text-[var(--brand-dark)]",
									children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("span", { className: "h-2 w-2 rounded-full bg-[#24a47d]" }), "Dentro do esperado"]
								})]
							}), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
								className: "p-6",
								children: [
									...
									/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
										...
										children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
											...
											children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(LineIcon, {
												...
												children: "Sinais vitais mais recentes"
											})]
										}), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("div", {
											className: "mt-3 grid grid-cols-2 gap-3",
											children: CARE_VITALS.map((item) => /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
												className: "rounded-xl bg-[var(--surface-soft)] p-4",
												children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", {
													className: "text-xs text-[var(--muted)]",
													children: item.label
												}), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("p", {
													className: "mt-1 text-lg font-semibold tracking-[-0.03em] text-[var(--foreground)]",
													children: [
														item.value,
														" ",
														/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("span", {
															className: "text-xs font-medium tracking-normal text-[var(--muted)]",
															children: item.unit
														})
													]
												})]
									...
									/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
										...
										children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("h4", {
											className: "text-sm font-semibold text-[var(--foreground)]",
											children: "Rotina do cuidado"
										}), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("div", {
											className: "mt-3 grid gap-3 sm:grid-cols-2",
											children: CARE_ROUTINE.map((item) => /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
												className: "flex items-center gap-3 rounded-xl border border-[var(--border)] px-4 py-3",
												children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("span", {
													className: "flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-[var(--brand-light)] text-[var(--brand-dark)]",
													children: /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(LineIcon, {
														name: item.icon,
														className: "h-4 w-4"
													})
									...
									/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
										className: "mt-5 rounded-xl border border-[#cbe9e3] bg-[var(--brand-light)] p-5",
										children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
											className: "flex items-center gap-2",
											children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(LineIcon, {
												name: "check",
												...
												children: "Última observação"
											})]
										}), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", {
											className: "mt-2 text-sm leading-6 text-[var(--muted)]",
											children: "Rotina realizada conforme o plano do período, com boa aceitação das refeições e caminhada assistida."
										})]
									}),
									/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
										className: "mt-5 flex flex-col gap-2 text-xs text-[var(--muted)] sm:flex-row sm:items-center sm:justify-between",
										children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("span", { children: "Registro ilustrativo do profissional responsável" }), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("span", {
											className: "inline-flex items-center gap-1.5 font-semibold text-[var(--brand)]",
											children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(LineIcon, {
												name: "clock",
												className: "h-3.5 w-3.5"
				...
				/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("section", {
					id: "contato",
					className: "home-contact bg-[#eafbf8] py-16 sm:py-20",
					children: /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("div", {
						className: "container-page relative z-[1]",
						children: /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
							className: "flex flex-col gap-7 rounded-3xl border border-[#bfe8e1] bg-white p-7 shadow-[0_16px_42px_rgba(27,82,75,0.08)] sm:p-10 lg:flex-row lg:items-center lg:justify-between",
							children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", { children: [
								/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("span", {
									className: "eyebrow",
								...
								/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("h2", {
									className: "mt-3 text-2xl font-bold tracking-[-0.04em] text-[var(--foreground)] sm:text-3xl",
									children: "Encontre o cuidado adequado para sua família"
								}),
								/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", {
									className: "mt-3 max-w-2xl text-sm leading-6 text-[var(--muted)] sm:text-base",
									children: "O formulário agora está em uma página própria, mais simples de preencher. Se preferir, você também pode conversar diretamente pelo WhatsApp."
								})
							] }), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
								...
								children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("a", {
									...
									children: ["Preencher solicitação", /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(LineIcon, {
										name: "arrow",
										className: "h-4 w-4"
									})]
								}), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("a", {
		...
		/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(PublicFooter, {})
	] });
}
//#endregion
...
function Card({ children, className = "" }) {
	return /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("div", {
		className: `rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-6 shadow-[0_12px_35px_rgba(8,54,49,0.05)] ${className}`,
		children
	});
}
//#endregion
//#region src/app/admin/page.tsx
var page_exports$17 = /* @__PURE__ */ __exportAll({ default: () => AdminHomePage });
		...
		children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("div", {
			...
			children: [
				...
				},
				{
					...
					value: patients.filter((p) => p.status === "pendente").length,
					href: "/admin/pacientes"
				},
				{
			...
			].map((s) => /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(link_default, {
				href: s.href,
			...
			children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)(Card, { children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
				...
				className: "space-y-3",
				children: [leads.slice(0, 5).map((lead) => /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
					className: "flex items-center justify-between text-sm border-b border-[var(--border)] pb-3 last:border-0 last:pb-0",
					children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", { children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", {
						...
						className: "text-[var(--muted-2)]",
						children: lead.phone
					})] }), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(Pill, { value: lead.status })]
				}, lead.id)), leads.length === 0 && /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", {
					className: "text-sm text-[var(--muted-2)]",
					children: "Nenhum contato recebido ainda."
				})]
			})] }), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)(Card, { children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
				className: "flex items-center justify-between mb-4",
				children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("h3", {
					className: "font-semibold text-[var(--foreground)]",
				...
				className: "space-y-3",
				children: [patients.slice(0, 5).map((p) => /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
					className: "flex items-center justify-between text-sm border-b border-[var(--border)] pb-3 last:border-0 last:pb-0",
					children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", { children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", {
						className: "font-medium text-[var(--foreground)]",
						children: p.name
						...
						className: "text-[var(--muted-2)]",
						children: p.care_level || "Plano a definir"
					})] }), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(Pill, { value: p.status })]
				}, p.id)), patients.length === 0 && /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", {
					className: "text-sm text-[var(--muted-2)]",
					children: "Nenhum paciente cadastrado ainda."
				})]
			})] })]
		})]
	});
...
//#endregion
//#region src/app/admin/layout.tsx
var layout_exports$1 = /* @__PURE__ */ __exportAll({ default: () => AdminLayout });
...
var CaregiverBank = /* @__PURE__ */ registerClientReference(() => {
	throw new Error("Unexpectedly client reference export 'CaregiverBank' is called on server");
}, "59ccb47cf7ae", "CaregiverBank");
//#endregion
//#region src/components/admin/NewCaregiverForm.tsx
var NewCaregiverForm = /* @__PURE__ */ registerClientReference(() => {
	throw new Error("Unexpectedly client reference export 'NewCaregiverForm' is called on server");
}, "89609f94f48b", "NewCaregiverForm");
//#endregion
//#region src/components/admin/ContractManager.tsx
var ContractManager = /* @__PURE__ */ registerClientReference(() => {
	throw new Error("Unexpectedly client reference export 'ContractManager' is called on server");
}, "b848232df12e", "ContractManager");
//#endregion
//#region src/components/admin/DeleteButton.tsx
var DeleteButton = /* @__PURE__ */ registerClientReference(() => {
	throw new Error("Unexpectedly client reference export 'DeleteButton' is called on server");
}, "a832555ecba8", "DeleteButton");
//#endregion
//#region src/app/admin/cuidadores/page.tsx
var page_exports$16 = /* @__PURE__ */ __exportAll({ default: () => AdminCaregiversPage });
async function AdminCaregiversPage() {
	const [profiles, caregiverUsers] = await Promise.all([listCaregiverProfiles(), listUsersByRole("cuidador")]);
	...
	const patientNamesByUser = new Map(caregiverUsers.map((caregiver, index) => [caregiver.id, patientLists[index].map((patient) => patient.name)]));
	const patientsByProfile = Object.fromEntries(profiles.map((profile) => [profile.id, profile.user_id ? patientNamesByUser.get(profile.user_id) || [] : []]));
	const contractsByProfile = Object.fromEntries(profiles.map((profile, index) => [profile.id, profileContractLists[index]]));
	const contractsByManualUser = Object.fromEntries(caregiverUsers.map((caregiver, index) => [caregiver.id, manualContractLists[index]]));
		...
		children: [
			/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
				...
				children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("h2", {
					className: "text-xl font-semibold text-[var(--foreground)]",
					children: "Banco de cuidadores e técnicos"
				}), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", {
					className: "mt-2 text-sm leading-6 text-[var(--muted)]",
					children: "Ao aprovar uma candidatura, o perfil entra automaticamente nesta lista. O acesso ao painel só é ativado quando você definir o e-mail e uma senha provisória."
				})]
			}),
			/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(CaregiverBank, {
				profiles,
				patientsByProfile,
				contractsByProfile
			}),
			/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("section", {
				...
				children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
					...
					children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", { children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("h2", {
						className: "font-semibold text-[var(--foreground)]",
						children: "Cadastros manuais"
					}), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", {
						className: "mt-1 text-sm text-[var(--muted)]",
						children: "Use somente quando o profissional não tiver enviado uma candidatura pelo site."
					})] }), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(NewCaregiverForm, {})]
				}), manuallyCreatedUsers.length > 0 ? /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("div", {
					...
					children: /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("table", {
						...
						children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("thead", { children: /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("tr", {
							...
							children: [
								...
								/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("th", {
									className: "px-5 py-3 font-medium",
									children: "Pacientes atribuídos"
								}),
								/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("th", {
									className: "px-5 py-3 font-medium",
									children: "Contratos e ações"
								})
							]
						}) }), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("tbody", { children: manuallyCreatedUsers.map((caregiver) => /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("tr", {
							className: "border-b border-[var(--border)] last:border-0",
							children: [
								/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("td", {
									className: "px-5 py-4 font-medium text-[var(--foreground)]",
									children: caregiver.name
								}),
								/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("td", {
								...
								/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("td", {
									...
									children: (patientNamesByUser.get(caregiver.id) || []).join(", ") || "Nenhum"
								}),
								/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("td", {
									className: "min-w-[380px] px-5 py-4",
									children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(ContractManager, {
										ownerType: "caregiver_user",
										ownerId: caregiver.id,
										contracts: contractsByManualUser[caregiver.id] || []
									}), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("div", {
										className: "mt-3",
										children: /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(DeleteButton, {
					...
					className: "rounded-xl bg-[var(--surface-soft)] p-4 text-sm text-[var(--muted-2)]",
					children: "Nenhum cuidador foi criado manualmente."
				})]
			})
		]
	});
...
//#endregion
//#region src/components/admin/FamilyAccountsTable.tsx
function FamilyAccountsTable({ families, patientsByFamily, contractsByFamily }) {
	if (!families.length) return /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("div", {
		className: "rounded-2xl border border-dashed border-[var(--border)] p-10 text-center text-sm text-[var(--muted)]",
		children: "Nenhuma conta de família cadastrada."
	});
	return /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("div", {
		className: "space-y-5",
				...
				children: [
					/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
						...
						children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", { children: [
							...
							/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", {
								className: "mt-1 text-sm text-[var(--muted-2)]",
								children: family.phone || "Telefone não informado"
							})
						] }), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(DeleteButton, {
					...
					/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
						...
						children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("h4", {
							className: "text-sm font-semibold text-[var(--foreground)]",
							children: "Pacientes vinculados"
						}), patients.length ? /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("div", {
							className: "mt-2 flex flex-wrap gap-2",
							children: patients.map((patient) => /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(link_default, {
							...
							className: "mt-2 text-sm text-[var(--muted-2)]",
							children: "Nenhum paciente vinculado."
						})]
					}),
					/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(ContractManager, {
						ownerType: "family",
						ownerId: family.id,
...
//#endregion
//#region src/app/admin/familias/page.tsx
var page_exports$15 = /* @__PURE__ */ __exportAll({ default: () => AdminFamiliesPage });
async function AdminFamiliesPage() {
	const [families, patients] = await Promise.all([listUsersByRole("familia"), listPatients()]);
	const contractEntries = await Promise.all(families.map(async (family) => [family.id, await listContractDocuments("family", family.id)]));
	const patientsByFamily = {};
	for (const patient of patients) {
		if (!patient.family_user_id) continue;
		(patientsByFamily[patient.family_user_id] ||= []).push(patient);
	}
	const contractsByFamily = Object.fromEntries(contractEntries);
	...
	return /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
		...
		children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
			...
			children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("h2", {
				...
				className: "mt-1 text-sm text-[var(--muted)]",
				children: "Gerencie os acessos, pacientes vinculados e contratos assinados de cada família."
			})]
		}), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(FamilyAccountsTable, {
...
var LeadsTable = /* @__PURE__ */ registerClientReference(() => {
	throw new Error("Unexpectedly client reference export 'LeadsTable' is called on server");
}, "f88a42f885ab", "LeadsTable");
//#endregion
//#region src/app/admin/leads/page.tsx
var page_exports$14 = /* @__PURE__ */ __exportAll({ default: () => AdminLeadsPage });
...
//#endregion
//#region src/app/admin/pacientes/page.tsx
var page_exports$13 = /* @__PURE__ */ __exportAll({ default: () => AdminPatientsPage });
async function AdminPatientsPage() {
	const patients = await listPatients();
	const familyUsers = await getUsersByIds(patients.map((p) => p.family_user_id).filter((id) => Boolean(id)));
	const familyById = new Map(familyUsers.map((u) => [u.id, u]));
	return /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", { children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
		className: "flex items-center justify-between mb-6",
		children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", {
			className: "text-sm text-[var(--muted)]",
			children: "Todos os pacientes cadastrados na Vellora Saúde."
		}), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(link_default, {
		...
		children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("table", {
			...
			}) }), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("tbody", { children: patients.map((p) => {
				const family = p.family_user_id ? familyById.get(p.family_user_id) : void 0;
				return /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("tr", {
					...
					children: [
						...
						/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("td", {
							className: "px-5 py-4 text-[var(--foreground)]",
							children: p.care_level || "—"
						}),
						/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("td", {
							className: "px-5 py-4 text-[var(--foreground)]",
							children: family ? `${family.name} (${family.email})` : "Não vinculada"
						}),
						/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("td", {
							className: "px-5 py-4",
							children: /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(Pill, { value: p.status })
						})
					]
				}, p.id);
			}) })]
		}), patients.length === 0 && /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", {
...
var NewPatientForm = /* @__PURE__ */ registerClientReference(() => {
	throw new Error("Unexpectedly client reference export 'NewPatientForm' is called on server");
}, "8b554f06f596", "NewPatientForm");
//#endregion
//#region src/app/admin/pacientes/novo/page.tsx
var page_exports$12 = /* @__PURE__ */ __exportAll({ default: () => NewPatientPage });
async function NewPatientPage({ searchParams }) {
	const params = await searchParams;
	const leadId = typeof params.lead === "string" ? params.lead : void 0;
	const [leads, familyUsers] = await Promise.all([listLeads(), listUsersByRole("familia")]);
	const lead = leadId ? leads.find((l) => l.id === leadId) : void 0;
	return /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
		className: "max-w-3xl",
		children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(link_default, {
			href: "/admin/pacientes",
			...
			children: [
				/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("h2", {
					className: "text-lg font-semibold text-[var(--foreground)]",
					children: "Cadastrar novo paciente"
				}),
				lead && /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("p", {
					className: "mt-1 text-sm text-[var(--muted)]",
					children: [
						"Convertendo o contato de ",
						/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("strong", { children: lead.name }),
						" (",
						lead.email,
						")."
					]
				...
				/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("div", {
					className: "mt-6",
					children: /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(NewPatientForm, {
						familyUsers,
						leadId: lead?.id,
						defaults: lead ? {
							patientName: lead.patient_name || void 0,
							familyName: lead.name,
							familyEmail: lead.email,
							familyPhone: lead.phone,
							careType: lead.care_type || void 0,
...
var ProfessionalApplicationsTable = /* @__PURE__ */ registerClientReference(() => {
	throw new Error("Unexpectedly client reference export 'ProfessionalApplicationsTable' is called on server");
}, "adcf62948e95", "ProfessionalApplicationsTable");
//#endregion
//#region src/app/admin/profissionais/page.tsx
var page_exports$11 = /* @__PURE__ */ __exportAll({ default: () => AdminProfessionalsPage });
async function AdminProfessionalsPage() {
...
async function POST$11(req) {
	...
	const startDate = body?.start_date || (/* @__PURE__ */ new Date()).toISOString().slice(0, 10);
	if (!patientId || !caregiverUserId) return NextResponse.json({ error: "Selecione paciente e cuidador." }, { status: 400 });
	try {
...
async function POST$10(req) {
	...
	if (password.length < 12) return NextResponse.json({ error: "A senha provisória deve ter pelo menos 12 caracteres." }, { status: 400 });
	if (await getUserByEmail(email.toLowerCase())) return NextResponse.json({ error: "Já existe um usuário com esse e-mail." }, { status: 400 });
...
async function POST$9(req) {
	...
	if (!OWNER_TYPES.includes(ownerType) || !ownerId || !(file instanceof File)) return NextResponse.json({ error: "Selecione o cadastro e o arquivo PDF." }, { status: 400 });
	if (file.size <= 0 || file.size > MAX_FILE_SIZE) return NextResponse.json({ error: "O contrato deve ter no máximo 4 MB." }, { status: 400 });
	const bytes = Buffer.from(await file.arrayBuffer());
	if (!(file.name.toLowerCase().endsWith(".pdf") && bytes.subarray(0, 5).toString() === "%PDF-")) return NextResponse.json({ error: "Envie somente um contrato em PDF válido." }, { status: 400 });
	if (ownerType === "family") {
		const user = await getUserById(ownerId);
		if (!user || user.role !== "familia" || user.deleted_at) return NextResponse.json({ error: "Conta da família não encontrada." }, { status: 404 });
	} else if (ownerType === "caregiver_profile") {
		if (!await getCaregiverProfile(ownerId)) return NextResponse.json({ error: "Cadastro profissional não encontrado." }, { status: 404 });
	} else {
		const user = await getUserById(ownerId);
		if (!user || user.role !== "cuidador" || user.deleted_at) return NextResponse.json({ error: "Conta do cuidador não encontrada." }, { status: 404 });
	}
	try {
...
async function POST$8(req) {
	...
	let familyUserId = body?.family_user_id || null;
	if (!familyUserId && body?.new_family_name && body?.new_family_email && body?.new_family_password) {
		if (body.new_family_password.length < 12) return NextResponse.json({ error: "A senha provisória deve ter pelo menos 12 caracteres." }, { status: 400 });
		if (await getUserByEmail(body.new_family_email.toLowerCase().trim())) return NextResponse.json({ error: "Já existe um usuário com esse e-mail de família." }, { status: 400 });
		familyUserId = (await createUser({
	...
	const patient = await createPatient({
		name,
		birth_date: body?.birth_date || void 0,
		address: body?.address || void 0,
		care_level: body?.care_level || void 0,
		condition_summary: body?.condition_summary || void 0,
...
function memoryRateLimit(key, options) {
	const now = Date.now();
	const current = memoryBuckets.get(key);
	const resetAt = current && current.resetAt > now ? current.resetAt : now + options.windowSeconds * 1e3;
	const bucket = current && current.resetAt > now ? current : {
		count: 0,
		resetAt
	};
	bucket.count += 1;
	memoryBuckets.set(key, bucket);
	if (memoryBuckets.size > MAX_MEMORY_BUCKETS) for (const [bucketKey, value] of memoryBuckets) {
...
async function consumeRateLimit(request, scope, options, discriminator = "") {
	...
	try {
       ...
       RETURNING count`, [bucketKey, options.windowSeconds]);
		const count = Number(row?.count || 1);
		if (windowIndex % 16 === 0) await query("DELETE FROM rate_limit_buckets WHERE expires_at <= CURRENT_TIMESTAMP").catch(() => void 0);
		return {
...
function parseOptionalNumber(value, options) {
	...
	if (!Number.isFinite(number) || number < options.min || number > options.max) return {
		valid: false,
		value: null
	};
	if (options.integer && !Number.isInteger(number)) return {
		valid: false,
		value: null
	};
	return {
...
async function POST$7(req) {
	...
	const body = await req.json().catch(() => null);
	const email = typeof body?.email === "string" ? body.email.trim().toLowerCase() : "";
	if (!email || !isValidEmail(email)) return NextResponse.json({ error: "Informe um e-mail válido." }, { status: 400 });
	const rate = await consumeRateLimit(req, "auth-forgot-password", {
		limit: 5,
		windowSeconds: 900
	}, email);
	if (!rate.allowed) return NextResponse.json({ error: "Muitas solicitações em pouco tempo. Aguarde alguns minutos e tente novamente." }, {
		status: 429,
...
async function POST$6(req) {
	...
	const password = typeof body?.password === "string" ? body.password : "";
	if (!email || !password || !isValidEmail(email)) return NextResponse.json({ error: "Informe e-mail e senha." }, { status: 400 });
	const rate = await consumeRateLimit(req, "auth-login", {
		limit: 10,
		windowSeconds: 900
	}, email);
	if (!rate.allowed) return NextResponse.json({ error: "Muitas tentativas de acesso. Aguarde alguns minutos e tente novamente." }, {
		status: 429,
		headers: { "Retry-After": String(rate.retryAfterSeconds) }
	});
	try {
		...
		console.error("[api/auth/login] Não foi possível consultar a conta.", { error: error instanceof Error ? error.message : "Erro desconhecido" });
		return NextResponse.json({ error: "A área restrita está temporariamente indisponível. Tente novamente em instantes." }, { status: 503 });
	}
}
//#endregion
...
async function POST$4(req) {
	...
	if (!rate.allowed) return NextResponse.json({ error: "Muitas tentativas em pouco tempo. Aguarde alguns minutos e tente novamente." }, {
		status: 429,
		headers: { "Retry-After": String(rate.retryAfterSeconds) }
	});
	if (getAuthProvider() === "supabase") try {
...
async function handlePublicLeadRequest(req, logLabel) {
	if (!isSameOriginRequest(req)) return NextResponse.json({ error: "Origem da solicitação não autorizada." }, { status: 403 });
	const rate = await consumeRateLimit(req, logLabel, {
		limit: 5,
		windowSeconds: 600
	});
	if (!rate.allowed) return NextResponse.json({ error: "Muitas solicitações em pouco tempo. Aguarde alguns minutos e tente novamente." }, {
	...
	const body = await req.json().catch(() => null);
	if (!body || typeof body !== "object") return NextResponse.json({ error: "Dados da solicitação inválidos." }, { status: 400 });
	...
	if (!name || !email || !phone || !patientName || !careType) return NextResponse.json({ error: "Preencha os campos obrigatórios da solicitação." }, { status: 400 });
	...
	try {
		...
		console.error(`[${logLabel}] Não foi possível gravar a solicitação.`, { error: error instanceof Error ? error.message : "Erro desconhecido" });
		return NextResponse.json({ error: "Não foi possível registrar a solicitação agora. Tente novamente em alguns instantes." }, { status: 503 });
	}
}
//#endregion
...
var PRIVACY_NOTICE_VERSION = "2026-08-21";
var VALID_DAYS = new Set([
	"segunda",
	"terca",
	"quarta",
	"quinta",
...
function cleanSelection(value, allowed) {
	if (!Array.isArray(value)) return [];
	return [...new Set(value.filter((item) => typeof item === "string" && allowed.has(item)))];
}
async function POST$2(req) {
	if (!isSameOriginRequest(req)) return NextResponse.json({ error: "Origem da solicitação não autorizada." }, { status: 403 });
	const rate = await consumeRateLimit(req, "professional-application", {
		limit: 3,
		windowSeconds: 600
	});
	if (!rate.allowed) return NextResponse.json({ error: "Muitas solicitações em pouco tempo. Aguarde alguns minutos e tente novamente." }, {
	...
	if (!name || !email || !phone || !city || !experience) return NextResponse.json({ error: "Preencha os campos obrigatórios." }, { status: 400 });
	if (!VALID_PROFESSIONS.has(profession)) return NextResponse.json({ error: "Selecione uma área profissional válida." }, { status: 400 });
	if (!isValidEmail(email)) return NextResponse.json({ error: "Informe um e-mail válido." }, { status: 400 });
	if (normalizedPhoneDigits(phone).length < 10) return NextResponse.json({ error: "Informe um telefone com DDD." }, { status: 400 });
	if ((profession === "tecnico_enfermagem" || profession === "enfermeiro") && !coren) return NextResponse.json({ error: "Informe o COREN para profissionais de enfermagem." }, { status: 400 });
	...
	try {
		...
		console.error("[api/professionals] Não foi possível gravar a candidatura.", { error: error instanceof Error ? error.message : "Erro desconhecido" });
		return NextResponse.json({ error: "Não foi possível enviar o cadastro agora. Tente novamente em alguns instantes." }, { status: 503 });
	}
}
//#endregion
...
function limitedText(form, key, maxLength) {
	return cleanText(form.get(key), maxLength) || null;
}
function validImageSignature(type, bytes) {
	if (type === "image/jpeg") return bytes[0] === 255 && bytes[1] === 216;
	if (type === "image/png") return bytes.length >= 8 && bytes.slice(0, 8).join(",") === "137,80,78,71,13,10,26,10";
...
async function readPhoto(form) {
	...
	if (photo.size > MAX_PHOTO_BYTES) return {
		provided: true,
		data: null,
		error: "Foto muito grande (máx. 3 MB)."
	};
	if (!ALLOWED_TYPES.has(photo.type)) return {
		provided: true,
		data: null,
		error: "Formato de imagem não suportado. Use JPG, PNG ou WEBP."
	};
	const bytes = new Uint8Array(await photo.arrayBuffer());
	if (!validImageSignature(photo.type, bytes)) return {
		provided: true,
		data: null,
	...
	return {
		provided: true,
		data: `data:${photo.type};base64,${buffer.toString("base64")}`
	};
}
function parseRecordFields(form) {
	const recordDate = limitedText(form, "record_date", 10);
	const recordTime = limitedText(form, "record_time", 5);
	if (!recordDate || !isValidIsoDate(recordDate)) return { error: "Informe uma data de medição válida." };
	if (!recordTime || !isValidTime(recordTime)) return { error: "Informe um horário de medição válido." };
	const numbers = {};
	...
	const incidentDescription = limitedText(form, "incident_description", 2e3);
	if (incident && !incidentDescription) return { error: "Descreva a intercorrência antes de salvar o registro." };
	return { values: {
		...
		medications: limitedText(form, "medications", 2e3),
		feeding: limitedText(form, "feeding", 2e3),
		hygiene: limitedText(form, "hygiene", 2e3),
		mobility: limitedText(form, "mobility", 2e3),
		mood,
...
async function saveRecord(req, mode) {
	...
	const rate = await consumeRateLimit(req, `records:${session.userId}`, {
		limit: 60,
		windowSeconds: 3600
	}, patientId);
	if (!rate.allowed) return rateLimitedResponse(rate.retryAfterSeconds);
	if (!patientId || !await isCaregiverAssignedToPatient(session.userId, patientId)) return NextResponse.json({ error: "Você não está vinculado a este paciente." }, { status: 403 });
	...
	try {
		...
		console.error(`[api/records/${mode}] Não foi possível salvar o registro.`, { error: error instanceof Error ? error.message : "Erro desconhecido" });
		return NextResponse.json({ error: "Não foi possível salvar o registro agora. Tente novamente em alguns instantes." }, { status: 503 });
	}
}
//#endregion
...
function ContractList({ contracts }) {
	return /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("section", {
		className: "mb-7 rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-5",
		children: [
			/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("h2", {
				className: "font-semibold text-[var(--foreground)]",
				children: "Meus contratos"
			}),
			/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", {
				className: "mt-1 text-sm text-[var(--muted)]",
				children: "Documentos assinados disponibilizados pela Vellora Saúde."
			}),
			contracts.length ? /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("div", {
				className: "mt-4 grid gap-3 sm:grid-cols-2",
				children: contracts.map((contract) => /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("a", {
					...
					children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", {
						className: "truncate text-sm font-semibold text-[var(--brand-dark)]",
				...
				children: "Nenhum contrato foi disponibilizado ainda."
			})
		]
	});
}
//#endregion
//#region src/app/cuidador/page.tsx
var page_exports$10 = /* @__PURE__ */ __exportAll({ default: () => CuidadorHomePage });
async function CuidadorHomePage() {
	const session = await getSession();
	const [patients, contracts] = session ? await Promise.all([listPatientsByCaregiver(session.userId), listContractDocumentsForCaregiver(session.userId)]) : [[], []];
	...
	return /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
		...
		children: [
			/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", {
				className: "text-sm text-[var(--muted)] mb-6",
				children: "Pacientes sob seus cuidados atualmente."
			}),
			/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(ContractList, { contracts }),
			patients.length === 0 && /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("div", {
				className: "rounded-2xl border border-dashed border-[var(--border)] p-10 text-center text-[var(--muted)]",
				children: "Você ainda não está vinculado(a) a nenhum paciente. Fale com a administração da Vellora Saúde."
			}),
			/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("div", {
				className: "space-y-4",
					...
					return /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
						...
						children: [
							/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
								...
								})] }), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(Pill, { value: patient.status })]
							}),
							/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", {
								className: "mt-2 text-xs text-[var(--muted-2)]",
								children: patient.condition_summary
							}),
							/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
								className: "mt-4 flex flex-wrap gap-3 border-t border-[var(--border)] pt-4",
								children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(link_default, {
									href: filledToday ? `/cuidador/paciente/${patient.id}/registro?recordId=${latest.id}` : `/cuidador/paciente/${patient.id}/registro`,
									className: `rounded-lg px-4 py-2 text-sm font-medium text-white ${filledToday ? "bg-[var(--accent)] hover:brightness-95" : "bg-[var(--brand)] hover:bg-[var(--brand-dark)]"}`,
									children: filledToday ? "✓ Editar registro de hoje" : "Preencher registro de hoje"
								}), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(link_default, {
...
}, "db858d0f5d88", "ForgotPasswordForm");
//#endregion
//#region src/app/esqueci-senha/page.tsx
var page_exports$9 = /* @__PURE__ */ __exportAll({ default: () => ForgotPasswordPage });
	...
	return /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("main", {
		...
		children: /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
			...
			children: [
				...
				/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("section", {
					className: "rounded-[1.75rem] border border-[var(--border)] bg-[var(--surface)] p-6 shadow-[0_20px_60px_rgba(8,54,49,0.08)] sm:p-9",
					children: [
						/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", {
							className: "text-xs font-bold uppercase tracking-[0.14em] text-[var(--accent-dark)]",
							children: "Recuperar acesso"
						}),
						/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("h1", {
							className: "font-display mt-3 text-4xl text-[var(--brand-deep)]",
							children: "Esqueceu sua senha?"
						}),
						/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", {
							className: "mt-3 text-sm leading-6 text-[var(--muted)]",
							children: "Informe o e-mail usado na área exclusiva. Se houver uma conta cadastrada, enviaremos um link seguro para criar uma nova senha."
						}),
						/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("div", {
							className: "mt-7",
							children: /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(ForgotPasswordForm, {})
						})
					]
				}),
				/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(link_default, {
					href: "/login",
					className: "mt-6 block text-center text-sm font-semibold text-[var(--brand)] hover:text-[var(--brand-dark)]",
...
//#endregion
//#region src/app/familia/page.tsx
var page_exports$8 = /* @__PURE__ */ __exportAll({ default: () => FamiliaHomePage });
async function FamiliaHomePage() {
	const session = await getSession();
	const [patients, contracts] = session ? await Promise.all([listPatientsByFamily(session.userId), listContractDocuments("family", session.userId)]) : [[], []];
	const latests = (await Promise.all(patients.map((p) => listRecordsForPatient(p.id, 1)))).map((records) => records[0]);
	...
	return /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
		...
		children: [
			/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", {
				className: "text-sm text-[var(--muted)] mb-6",
				children: "Acompanhe aqui o cuidado dos seus familiares atendidos pela Vellora Saúde."
			}),
			/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(ContractList, { contracts }),
			patients.length === 0 && /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("div", {
				className: "rounded-2xl border border-dashed border-[var(--border)] p-10 text-center text-[var(--muted)]",
				children: "Nenhum familiar vinculado à sua conta ainda. Assim que o contrato for confirmado pela nossa equipe, ele aparecerá aqui."
			}),
			/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("div", {
				className: "space-y-4",
				children: patients.map((patient, i) => {
					const latest = latests[i];
					const overall = latest ? worstStatus(statusBP(latest.bp_systolic, latest.bp_diastolic), statusHeartRate(latest.heart_rate), statusTemperature(latest.temperature), statusSpo2(latest.spo2), statusGlucose(latest.glucose)) : null;
					return /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)(link_default, {
						...
						children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
							...
							children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", { children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", {
								...
								className: "text-sm text-[var(--muted-2)]",
								children: patient.care_level || "Plano de cuidado a definir"
							})] }), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
								className: "flex items-center gap-2",
								children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(Pill, { value: patient.status }), overall && /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(StatusBadge, { status: overall })]
							})]
						}), latest ? /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("p", {
							className: "mt-4 text-sm text-[var(--muted)] border-t border-[var(--border)] pt-4",
							children: [
								"Último registro em ",
								(/* @__PURE__ */ new Date(latest.record_date + "T00:00:00")).toLocaleDateString("pt-BR"),
								" por",
								" ",
							...
							className: "mt-4 text-sm text-[var(--muted-2)] border-t border-[var(--border)] pt-4",
							children: "Ainda não há registros diários para este paciente."
						})]
					}, patient.id);
				})
			})
...
//#endregion
//#region src/app/familia/layout.tsx
var layout_exports = /* @__PURE__ */ __exportAll({ default: () => FamiliaLayout });
async function FamiliaLayout({ children }) {
	return /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(DashboardShell, {
...
}, "ee11ef59f376", "LoginForm");
//#endregion
//#region src/app/login/page.tsx
var page_exports$7 = /* @__PURE__ */ __exportAll({ default: () => LoginPage });
async function LoginPage({ searchParams }) {
	const params = await searchParams;
	const next = typeof params.next === "string" ? params.next : void 0;
	...
	return /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("main", {
		className: "grid min-h-screen bg-[var(--surface)] lg:grid-cols-[0.9fr_1.1fr]",
		children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("section", {
			className: "relative hidden overflow-hidden bg-[var(--brand-deep)] p-12 text-white lg:flex lg:flex-col lg:justify-between",
			children: [
				/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("div", { className: "absolute -right-24 -top-24 h-96 w-96 rounded-full border border-white/10" }),
				/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("div", { className: "absolute -bottom-32 -left-20 h-96 w-96 rounded-full bg-[var(--brand)]/35 blur-3xl" }),
				/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("div", {
					className: "relative inline-flex self-start rounded-xl bg-white p-3",
					children: /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(Brand, {})
				}),
				/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
					...
					children: [
						/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", {
							className: "text-xs font-bold uppercase tracking-[0.16em] text-[#e0bd7d]",
							children: "Área exclusiva"
						}),
						/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("h1", {
							className: "font-display mt-5 text-5xl leading-[1.05]",
							children: "O cuidado do dia a dia, organizado para quem precisa acompanhar."
						}),
						/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", {
							className: "mt-6 text-base leading-7 text-white/68",
							children: "Famílias, profissionais e gestão conectados em um ambiente com acesso restrito."
						})
					]
				}),
				/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", {
					className: "relative text-xs text-white/40",
					children: "Vellora Saúde · Goiânia e região"
				})
			]
		}), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("section", {
			className: "flex flex-col items-center justify-center px-5 py-12 sm:px-10",
			children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("div", {
				...
				children: [
					...
					/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("h2", {
						className: "font-display mt-3 text-4xl text-[var(--brand-deep)]",
						children: "Entre na sua área"
					}),
					/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", {
						className: "mt-3 text-sm leading-6 text-[var(--muted)]",
						children: safePreview ? "Esta prévia apresenta apenas o site institucional, sem expor contas ou informações de pacientes." : "Use o acesso fornecido pela equipe da Vellora para consultar as informações do seu perfil."
					}),
					/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
						className: "mt-8 rounded-[1.5rem] border border-[var(--border)] bg-[var(--surface)] p-6 shadow-[0_20px_60px_rgba(8,54,49,0.08)] sm:p-8",
						children: [passwordChanged && /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", {
							className: "mb-5 rounded-xl bg-[var(--brand-light)] p-4 text-sm font-semibold text-[var(--brand-deep)]",
							role: "status",
							children: "Senha alterada com sucesso. Entre com a nova senha."
						}), safePreview ? /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
							className: "rounded-xl border border-[var(--border)] bg-[var(--brand-light)] p-6",
							children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", {
								className: "font-semibold text-[var(--brand-deep)]",
								children: "Prévia segura"
							}), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", {
								className: "mt-2 text-sm leading-6 text-[var(--muted)]",
								children: "O login e os painéis de administrador, família e cuidador foram desativados neste endereço de avaliação. Nenhum prontuário ou dado de contato é exibido."
							})]
						}) : /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(LoginForm, { next })]
					}),
					/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
						className: "mt-6 flex items-center justify-between gap-4 text-sm",
						children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(link_default, {
							href: "/",
							className: "font-semibold text-[var(--muted)] hover:text-[var(--brand-dark)]",
...
//#endregion
//#region src/app/privacidade/page.tsx
			...
			children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("section", {
				...
				children: /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
					...
					children: [
						...
						/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("h1", {
							className: "section-title mt-4 max-w-3xl",
							children: "Política de Privacidade"
						}),
						/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", {
							className: "mt-5 max-w-2xl text-sm leading-7 text-[var(--muted)] sm:text-base",
							children: "Última atualização: 26 de agosto de 2026. Esta política explica como a Vellora Saúde trata dados pessoais nos seus canais de contato e áreas de acesso restrito."
						})
					]
				})
			}), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("article", {
				className: "container-page max-w-4xl py-12 sm:py-16",
				children: /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
					className: "space-y-9 rounded-2xl border border-[var(--border)] bg-white p-6 shadow-[0_12px_35px_rgba(27,82,75,0.06)] sm:p-10",
					children: [
						/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)(LegalSection$1, {
							title: "1. Escopo",
							children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", { children: "Esta política se aplica ao site da Vellora Saúde, aos formulários de solicitação de cuidado e de cadastro profissional e às áreas restritas destinadas a administradores, famílias e cuidadores." }), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", { children: "Os dados são tratados de acordo com a Lei Geral de Proteção de Dados (Lei nº 13.709/2018), considerando a finalidade de cada coleta e o nível de sensibilidade das informações." })]
						}),
						/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)(LegalSection$1, {
							...
							children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("ul", {
								...
								children: [
									/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("li", { children: "Solicitações de cuidado: nome, e-mail, telefone, nome do paciente, cidade, tipo e rotina de cuidado e a mensagem enviada." }),
									/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("li", { children: "Cadastros profissionais: nome, contatos, cidade, profissão, COREN quando aplicável, experiência, disponibilidade e observações profissionais." }),
									/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("li", { children: "Acesso restrito: dados da conta, vínculos de atendimento, registros de cuidado, sinais vitais, intercorrências, documentos contratuais e fotos anexadas ao atendimento." }),
									/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("li", { children: "Dados técnicos mínimos necessários para segurança, prevenção de abuso e funcionamento do site." })
								]
							}), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", { children: "Evite enviar documentos ou informações clínicas sensíveis nos formulários públicos. Use a área restrita ou aguarde a orientação da equipe." })]
						}),
						/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(LegalSection$1, {
							title: "3. Para que usamos os dados",
							children: /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("ul", {
								className: "list-disc space-y-2 pl-5",
								children: [
									/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("li", { children: "Entender a necessidade da família e retornar sobre uma solicitação de cuidado." }),
									/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("li", { children: "Analisar perfis profissionais e entrar em contato sobre oportunidades compatíveis." }),
									/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("li", { children: "Organizar escalas, vínculos, contratos e registros de atendimento autorizados." }),
									/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("li", { children: "Proteger contas, detectar tentativas abusivas, manter trilhas de auditoria e cumprir obrigações legais." })
								]
							})
						}),
						/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)(LegalSection$1, {
							title: "4. Bases legais e dados de saúde",
							children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", { children: "A base legal é definida conforme a atividade: consentimento, procedimentos preliminares relacionados a contrato, cumprimento de obrigação legal, exercício regular de direitos ou proteção da vida e da saúde quando aplicável." }), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", { children: "Dados referentes à saúde são tratados como sensíveis. O acesso é limitado ao que cada perfil precisa para cumprir sua função, e o compartilhamento ocorre somente quando necessário para a prestação do cuidado, para cumprir a lei ou com autorização válida." })]
						}),
						/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)(LegalSection$1, {
							title: "5. Compartilhamento e armazenamento",
							children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", { children: "Podemos usar fornecedores de infraestrutura, armazenamento e envio de e-mails para operar o serviço. Eles recebem apenas os dados necessários para a finalidade contratada e devem manter medidas de segurança adequadas." }), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", { children: "Não vendemos dados pessoais. O armazenamento pode ocorrer em serviços de nuvem, inclusive fora do Brasil, observadas as garantias exigidas pela legislação aplicável." })]
						}),
						/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)(LegalSection$1, {
							title: "6. Segurança e retenção",
							children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", { children: "Adotamos controle de acesso por perfil, autenticação, proteção de sessão, validações nos formulários, limitação de tentativas e registro de alterações em dados de cuidado. Nenhuma transmissão ou armazenamento é absolutamente invulnerável." }), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", { children: "Os dados são mantidos pelo período necessário às finalidades informadas, à continuidade do cuidado, à defesa de direitos e às obrigações legais. Depois disso, podem ser excluídos, anonimizados ou mantidos de forma restrita quando houver justificativa." })]
						}),
						/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)(LegalSection$1, {
							title: "7. Seus direitos",
							children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", { children: "Você pode solicitar confirmação de tratamento, acesso, correção, informação sobre compartilhamento, portabilidade quando aplicável, anonimização, bloqueio ou eliminação de dados tratados com base legal que permita o pedido e revogação do consentimento." }), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("p", { children: [
								"Para solicitar atendimento, use o ",
								/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("a", {
									className: "font-semibold text-[var(--brand)] underline",
									href: "https://wa.me/5562981355553",
									target: "_blank",
									rel: "noopener noreferrer",
									children: "WhatsApp da Vellora Saúde"
								}),
								". Podemos pedir informações para confirmar sua identidade e preservar a segurança dos dados."
							] })]
						}),
						/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)(LegalSection$1, {
							title: "8. Atualizações",
							children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", { children: "Esta política pode ser atualizada para refletir mudanças no serviço, na legislação ou nas medidas de segurança. A data no início da página indica a versão vigente." }), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("p", { children: [
								"Para entender as regras de uso do site e do portal, consulte também os ",
								/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(link_default, {
									className: "font-semibold text-[var(--brand)] underline",
									href: "/termos",
									children: "Termos de Uso"
...
function LegalSection$1({ title, children }) {
	return /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("section", {
		className: "space-y-3 text-sm leading-7 text-[var(--muted)]",
		children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("h2", {
			className: "text-lg font-semibold leading-6 text-[var(--foreground)]",
			children: title
		}), children]
	});
}
...
}, "a1e5201a4cb5", "ResetPasswordForm");
//#endregion
//#region src/app/redefinir-senha/page.tsx
var page_exports$5 = /* @__PURE__ */ __exportAll({ default: () => ResetPasswordPage });
			...
			children: [
				...
				/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("section", {
					...
					children: [
						...
						/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("h1", {
							className: "font-display mt-3 text-4xl text-[var(--brand-deep)]",
							children: "Crie uma senha segura"
						}),
						/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", {
							className: "mt-3 text-sm leading-6 text-[var(--muted)]",
							children: "Use pelo menos 12 caracteres. Depois da troca, os acessos anteriores serão encerrados."
						}),
						/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("div", {
							className: "mt-7",
...
}, "df78bc062d1f", "ContactForm");
//#endregion
//#region src/app/solicitar-cuidado/page.tsx
var page_exports$4 = /* @__PURE__ */ __exportAll({
	default: () => RequestCarePage,
	metadata: () => metadata$2
});
var metadata$2 = {
			...
			children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("section", {
				...
				children: /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
					...
					children: [
						/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("span", {
							className: "eyebrow",
							children: "Fale com a Vellora Saúde"
						}),
						/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("h1", {
							className: "section-title mt-4 max-w-3xl",
							children: "Conte como podemos ajudar sua família"
						}),
						/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", {
							className: "mt-5 max-w-2xl text-base leading-7 text-[var(--muted)] sm:text-lg",
							children: "Preencha os dados essenciais para que nossa equipe entenda a necessidade e entre em contato. Você não precisa enviar documentos ou informações médicas neste primeiro momento."
						})
					]
				})
			}), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("section", {
				className: "container-page grid items-start gap-10 py-12 sm:py-16 lg:grid-cols-[0.78fr_1.22fr] lg:gap-16",
				children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("aside", {
					className: "space-y-4 lg:sticky lg:top-28",
					children: [CONTACT_POINTS.map((item) => /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("article", {
						className: "flex gap-4 rounded-2xl border border-[var(--border)] bg-white p-5",
						children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("span", {
							className: "flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-[var(--brand-dark)] text-white",
							children: /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(LineIcon, {
								name: item.icon,
								className: "h-5 w-5"
							})
					...
					}, item.title)), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
						className: "rounded-2xl border border-[#cbe9e3] bg-[var(--brand-light)] p-5",
						children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", {
							className: "text-sm font-semibold text-[var(--brand-deep)]",
							children: "Prefere conversar?"
						}), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", {
							className: "mt-2 text-sm leading-6 text-[var(--muted)]",
							children: "Use o botão do WhatsApp no canto da tela para falar diretamente com a empresa."
						})]
					})]
				}), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
					className: "rounded-2xl border border-[var(--border-strong)] bg-white p-6 shadow-[0_16px_42px_rgba(27,82,75,0.09)] sm:p-8",
					children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
						className: "mb-7",
						children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("h2", {
							className: "text-xl font-semibold text-[var(--foreground)]",
							children: "Formulário de solicitação"
						}), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", {
							className: "mt-1 text-sm text-[var(--muted)]",
...
//#endregion
//#region src/app/termos/page.tsx
			...
			children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("section", {
				...
				children: /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
					...
					children: [
						...
						/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", {
							className: "mt-5 max-w-2xl text-sm leading-7 text-[var(--muted)] sm:text-base",
							children: "Última atualização: 26 de agosto de 2026. Ao usar o site ou uma área restrita da Vellora Saúde, você concorda com estas regras."
						})
					]
				})
			}), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("article", {
				...
				children: /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
					className: "space-y-9 rounded-2xl border border-[var(--border)] bg-white p-6 shadow-[0_12px_35px_rgba(27,82,75,0.06)] sm:p-10",
					children: [
						/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)(LegalSection, {
							title: "1. O serviço",
							children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", { children: "A Vellora Saúde utiliza este site para apresentar serviços de cuidado domiciliar, receber solicitações de atendimento e cadastrar profissionais interessados em oportunidades." }), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", { children: "O envio de um formulário inicia uma conversa. Ele não confirma contratação, escala, diagnóstico, vaga ou prazo de atendimento." })]
						}),
						/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)(LegalSection, {
							title: "2. Acesso ao portal",
							children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", { children: "As áreas de administrador, família e cuidador são restritas a pessoas autorizadas pela equipe. A conta é pessoal e não deve ser compartilhada." }), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("ul", {
								className: "list-disc space-y-2 pl-5",
								children: [
									/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("li", { children: "Use uma senha forte e mantenha seus dados de acesso em sigilo." }),
									/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("li", { children: "Avise a equipe imediatamente se suspeitar de acesso indevido." }),
									/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("li", { children: "Consulte apenas as informações necessárias para sua função." }),
									/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("li", { children: "Não copie, baixe, divulgue ou use dados de pacientes fora da finalidade autorizada." })
								]
							})]
						}),
						/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)(LegalSection, {
							title: "3. Registros de cuidado",
							children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", { children: "Os registros devem refletir o atendimento realizado de forma objetiva e respeitosa. O cuidador é responsável por conferir os dados antes de salvar e corrigir eventuais erros pelo próprio portal ou comunicar a administração." }), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", { children: "O portal não substitui avaliação clínica, prescrição, orientação de emergência ou contato com os serviços públicos de urgência. Em situação de risco imediato, procure o serviço de emergência adequado." })]
						}),
						/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)(LegalSection, {
							title: "4. Formulários públicos",
							children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", { children: "Informe dados verdadeiros, atualizados e necessários. Não envie documentos, senhas, dados bancários ou informações clínicas detalhadas nos formulários públicos. A equipe indicará um canal apropriado quando esses dados forem indispensáveis." }), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", { children: "É proibido automatizar envios, tentar contornar mecanismos de segurança ou enviar conteúdo ilícito, ofensivo ou que viole direitos de terceiros." })]
						}),
						/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)(LegalSection, {
							title: "5. Disponibilidade e segurança",
							children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", { children: "Buscamos manter o serviço disponível e seguro, mas podem ocorrer manutenções, falhas de rede ou indisponibilidade de fornecedores. Podemos suspender contas ou acessos quando necessário para proteger pessoas, dados ou a operação." }), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("p", { children: [
								"As regras sobre coleta e tratamento de dados estão na ",
								/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(link_default, {
									className: "font-semibold text-[var(--brand)] underline",
									href: "/privacidade",
									children: "Política de Privacidade"
						...
						/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(LegalSection, {
							title: "6. Conteúdo e propriedade",
							children: /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", { children: "Textos, marca, identidade visual e materiais do site pertencem à Vellora Saúde ou são usados com autorização. O conteúdo não pode ser reproduzido para fins comerciais sem autorização prévia." })
						}),
						/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)(LegalSection, {
							title: "7. Alterações e contato",
							children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", { children: "Podemos atualizar estes termos para acompanhar mudanças no serviço ou na legislação. A data no início da página identifica a versão vigente." }), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("p", { children: [
								"Em caso de dúvida sobre o serviço ou sobre seu acesso, fale com a equipe pelo ",
								/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("a", {
									className: "font-semibold text-[var(--brand)] underline",
									href: "https://wa.me/5562981355553",
									target: "_blank",
...
var ProfessionalApplicationForm = /* @__PURE__ */ registerClientReference(() => {
	throw new Error("Unexpectedly client reference export 'ProfessionalApplicationForm' is called on server");
}, "b67ba796ee67", "ProfessionalApplicationForm");
//#endregion
//#region src/app/trabalhe-conosco/page.tsx
...
var metadata = {
	...
	description: "Cadastre seu perfil para oportunidades como cuidador, técnico de enfermagem, enfermeiro ou outro profissional na Vellora Saúde.",
	robots: {
		index: true,
		follow: true
	}
};
var PROFESSIONAL_BENEFITS = [
	{
...
function WorkWithUsPage() {
	return /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)(import_jsx_runtime_react_server.Fragment, { children: [
		/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("header", {
			className: "border-b border-[var(--border)] bg-white",
			children: /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
				className: "container-page flex min-h-[4.5rem] items-center justify-between gap-5 py-3",
				children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(Brand, {}), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)(link_default, {
					href: "/",
			...
			children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("section", {
				...
				children: /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
					...
					children: [
						/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("span", {
							className: "eyebrow",
							children: "Oportunidades profissionais"
						}),
						/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("h1", {
							className: "section-title mt-4 max-w-3xl",
							children: "Faça parte da rede de cuidado da Vellora Saúde"
						}),
						/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", {
							className: "mt-5 max-w-2xl text-base leading-7 text-[var(--muted)] sm:text-lg",
							children: "Esta página é destinada a cuidadores, técnicos de enfermagem, enfermeiros e outros profissionais interessados em atendimentos domiciliares em Goiânia e região."
						})
					]
				})
			}), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("section", {
				className: "container-page grid items-start gap-10 py-14 sm:py-16 lg:grid-cols-[0.78fr_1.22fr] lg:gap-16",
				children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("aside", {
					className: "lg:sticky lg:top-8",
					children: [
						/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("h2", {
							className: "text-xl font-semibold text-[var(--foreground)]",
							children: "Antes de preencher"
						}),
						/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", {
							className: "mt-3 text-sm leading-6 text-[var(--muted)]",
							children: "Informe seus dados profissionais e sua disponibilidade real. O cadastro não cria acesso ao painel e não garante contratação imediata."
						}),
						/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("div", {
							className: "mt-7 space-y-4",
							children: PROFESSIONAL_BENEFITS.map((item) => /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("article", {
								className: "flex gap-4 rounded-xl border border-[var(--border)] bg-white p-4",
								children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("span", {
									className: "flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-[var(--brand-dark)] text-white",
									children: /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(LineIcon, {
										name: item.icon,
										className: "h-5 w-5"
									})
						...
						/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
							className: "mt-6 rounded-xl border border-[#cbe9e3] bg-[var(--brand-light)] p-5",
							children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", {
								className: "text-sm font-semibold text-[var(--brand-deep)]",
								children: "Privacidade dos seus dados"
							}), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", {
								className: "mt-2 text-sm leading-6 text-[var(--muted)]",
								children: "Não envie documentos pessoais neste formulário. Caso seu perfil avance, a equipe explicará os próximos passos diretamente a você."
							})]
						})
					]
				}), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
					className: "rounded-xl border border-[var(--border-strong)] bg-white p-6 shadow-[0_12px_35px_rgba(27,82,75,0.08)] sm:p-8",
					children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
						className: "mb-7",
						children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("h2", {
							className: "text-lg font-semibold text-[var(--foreground)]",
							children: "Cadastro profissional"
						}), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", {
							className: "mt-1 text-sm text-[var(--muted)]",
							children: "Campos com * são obrigatórios."
						})]
...
var EditPatientForm = /* @__PURE__ */ registerClientReference(() => {
	throw new Error("Unexpectedly client reference export 'EditPatientForm' is called on server");
}, "41efcf7ef052", "EditPatientForm");
//#endregion
//#region src/components/admin/AssignmentsManager.tsx
var AssignmentsManager = /* @__PURE__ */ registerClientReference(() => {
	throw new Error("Unexpectedly client reference export 'AssignmentsManager' is called on server");
}, "1b3fb4a67ee2", "AssignmentsManager");
//#endregion
//#region src/components/admin/RecordAuditTimeline.tsx
...
function RecordAuditTimeline({ events }) {
	if (events.length === 0) return /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", {
		className: "text-sm text-[var(--muted-2)]",
		children: "Nenhuma alteração registrada ainda."
	});
	return /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("ol", {
		className: "space-y-3",
		children: events.map((event) => {
			const fields = event.changed_fields.map((field) => DAILY_RECORD_FIELD_LABELS[field] || field).join(", ");
			return /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("li", {
				...
				children: [
					/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
						...
						children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", {
							className: "font-semibold text-[var(--foreground)]",
							children: event.action === "created" ? "Registro criado" : "Registro atualizado"
					...
					/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("p", {
						...
						children: ["Por ", event.actor_name]
					}),
					fields && /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("p", {
...
//#endregion
//#region src/app/admin/pacientes/[id]/page.tsx
var page_exports$1 = /* @__PURE__ */ __exportAll({ default: () => AdminPatientDetailPage });
async function AdminPatientDetailPage({ params }) {
	const { id } = await params;
	const patient = await getPatient(id);
	if (!patient) notFound();
	const [currentFamily, familyUsers, caregivers, assignmentsRaw, records, auditEvents] = await Promise.all([
		patient.family_user_id ? getUserById(patient.family_user_id) : Promise.resolve(void 0),
	...
	const namesMap = await getCaregiverNamesMap([...assignmentsRaw.map((a) => a.caregiver_user_id), ...records.map((r) => r.caregiver_user_id)]);
	const assignments = assignmentsRaw.map((a) => ({
		...a,
		caregiverName: namesMap[a.caregiver_user_id] || "Cuidador"
	}));
	return /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
		className: "max-w-4xl",
		children: [
			...
			/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
				className: "mt-4 mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between",
				children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("h2", {
					className: "text-lg font-semibold text-[var(--foreground)]",
					children: patient.name
				}), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(DeleteButton, {
			...
			/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
				...
				children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)(Card, { children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("h3", {
					className: "font-semibold text-[var(--foreground)] mb-4",
					children: "Editar paciente"
				}), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(EditPatientForm, {
					patient,
					familyUsers,
					currentFamily
				})] }), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)(Card, { children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("h3", {
					className: "font-semibold text-[var(--foreground)] mb-4",
					children: "Cuidadores vinculados"
				}), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(AssignmentsManager, {
					patientId: patient.id,
					assignments,
					caregivers
				})] })]
			...
			/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("h3", {
				className: "mt-10 mb-4 text-lg font-semibold text-[var(--foreground)]",
				children: "Histórico de alterações"
			}),
			/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(RecordAuditTimeline, { events: auditEvents }),
			/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("h3", {
				className: "mt-10 mb-4 text-lg font-semibold text-[var(--foreground)]",
				children: "Últimos registros diários"
			}),
			/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
				className: "space-y-4",
				children: [records.length === 0 && /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", {
...
//#endregion
//#region src/app/api/contracts/[id]/route.ts
...
async function GET(_req, { params }) {
	...
	if (session.role === "familia") authorized = contract.family_user_id === session.userId;
	if (session.role === "cuidador") {
		authorized = contract.caregiver_user_id === session.userId;
		if (!authorized && contract.caregiver_profile_id) authorized = (await getCaregiverProfile(contract.caregiver_profile_id))?.user_id === session.userId;
	}
	...
	return new NextResponse(new Uint8Array(fileData), { headers: {
...
//#endregion
//#region src/components/charts/VitalsChart.tsx
var BloodPressureChart = /* @__PURE__ */ registerClientReference(() => {
	throw new Error("Unexpectedly client reference export 'BloodPressureChart' is called on server");
}, "f16d7dcb5c6b", "BloodPressureChart");
var HeartRateChart = /* @__PURE__ */ registerClientReference(() => {
	throw new Error("Unexpectedly client reference export 'HeartRateChart' is called on server");
}, "f16d7dcb5c6b", "HeartRateChart");
var TemperatureChart = /* @__PURE__ */ registerClientReference(() => {
	throw new Error("Unexpectedly client reference export 'TemperatureChart' is called on server");
}, "f16d7dcb5c6b", "TemperatureChart");
var Spo2Chart = /* @__PURE__ */ registerClientReference(() => {
	throw new Error("Unexpectedly client reference export 'Spo2Chart' is called on server");
}, "f16d7dcb5c6b", "Spo2Chart");
var GlucoseChart = /* @__PURE__ */ registerClientReference(() => {
	throw new Error("Unexpectedly client reference export 'GlucoseChart' is called on server");
}, "f16d7dcb5c6b", "GlucoseChart");
//#endregion
//#region src/app/familia/paciente/[id]/page.tsx
var page_exports = /* @__PURE__ */ __exportAll({ default: () => FamiliaPatientPage });
async function FamiliaPatientPage({ params }) {
	const { id } = await params;
	const session = await getSession();
	const patient = await getPatient(id);
	if (!patient || !session || patient.family_user_id !== session.userId) notFound();
	...
	const namesMap = await getCaregiverNamesMap([...activeAssignments.map((a) => a.caregiver_user_id), ...records.map((r) => r.caregiver_user_id)]);
	...
	return /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
		...
		children: [
			/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(link_default, {
				href: "/familia",
				className: "text-sm text-[var(--muted)] hover:text-[var(--foreground)]",
				children: "← Meus familiares"
			}),
			/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
				className: "mt-4 rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-6",
				children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
					className: "flex flex-wrap items-center justify-between gap-3",
					children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", { children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("h2", {
						className: "text-xl font-semibold text-[var(--foreground)]",
					...
					className: "mt-4 grid sm:grid-cols-3 gap-4 text-sm border-t border-[var(--border)] pt-4",
					children: [
						/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", { children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", {
							className: "text-[var(--muted-2)]",
							children: "Plano"
						...
						/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", { children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", {
							className: "text-[var(--muted-2)]",
							children: "Cuidador(es) responsável(is)"
						}), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", {
							className: "text-[var(--foreground)]",
							children: caregiverNames.join(", ") || "A definir"
						})] })
					]
				})]
			}),
			chartData.length > 0 && /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)(import_jsx_runtime_react_server.Fragment, { children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("h3", {
				className: "mt-10 mb-4 text-lg font-semibold text-[var(--foreground)]",
				children: "Evolução dos sinais vitais"
			}), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
				className: "grid md:grid-cols-2 gap-5",
				children: [
					/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(BloodPressureChart, { data: chartData }),
					/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(HeartRateChart, { data: chartData }),
					/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(TemperatureChart, { data: chartData }),
					/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(Spo2Chart, { data: chartData }),
					/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(GlucoseChart, { data: chartData })
				]
			})] }),
			/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("h3", {
				className: "mt-10 mb-4 text-lg font-semibold text-[var(--foreground)]",
				children: "Histórico de registros diários"
			}),
			/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("div", {
				className: "space-y-4",
				children: [records.length === 0 && /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("p", {
...
var routes = [
	{
		...
		ids: {
			"route": "route:/api/admin/caregivers/:id/access",
			"page": null,
			"routeHandler": "route-handler:/api/admin/caregivers/:id/access",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
		},
		pattern: "/api/admin/caregivers/:id/access",
		patternParts: [
			"api",
			"admin",
			"caregivers",
	...
	},
	{
		...
		ids: {
			"route": "route:/cuidador/paciente/:id/historico",
			"page": "page:/cuidador/paciente/:id/historico",
			"routeHandler": null,
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/", "layout:/cuidador"],
			"templates": [],
			"slots": {}
		},
		pattern: "/cuidador/paciente/:id/historico",
		patternParts: [
			"cuidador",
			"paciente",
			":id",
	...
	},
	{
		...
		ids: {
			"route": "route:/cuidador/paciente/:id/registro",
			"page": "page:/cuidador/paciente/:id/registro",
			"routeHandler": null,
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/", "layout:/cuidador"],
			"templates": [],
		...
		pattern: "/cuidador/paciente/:id/registro",
		patternParts: [
			"cuidador",
			"paciente",
			":id",
	...
	},
	{
		...
		ids: {
			"route": "route:/api/admin/assignments/:id",
			"page": null,
			"routeHandler": "route-handler:/api/admin/assignments/:id",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
		},
		pattern: "/api/admin/assignments/:id",
		patternParts: [
			"api",
			"admin",
			"assignments",
	...
	},
	{
		...
		ids: {
			"route": "route:/api/admin/caregiver-users/:id",
			"page": null,
			"routeHandler": "route-handler:/api/admin/caregiver-users/:id",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
		},
		pattern: "/api/admin/caregiver-users/:id",
		patternParts: [
			"api",
			"admin",
			"caregiver-users",
	...
	},
	{
		...
		ids: {
			"route": "route:/api/admin/caregivers/:id",
			"page": null,
			"routeHandler": "route-handler:/api/admin/caregivers/:id",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
		},
		pattern: "/api/admin/caregivers/:id",
		patternParts: [
			"api",
			"admin",
			"caregivers",
	...
	},
	{
		...
		ids: {
			"route": "route:/api/admin/contracts/:id",
			"page": null,
			"routeHandler": "route-handler:/api/admin/contracts/:id",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
		},
		pattern: "/api/admin/contracts/:id",
		patternParts: [
			"api",
			"admin",
			"contracts",
	...
	},
	{
		...
		ids: {
			"route": "route:/api/admin/families/:id",
			"page": null,
			"routeHandler": "route-handler:/api/admin/families/:id",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
		},
		pattern: "/api/admin/families/:id",
		patternParts: [
			"api",
			"admin",
			"families",
	...
	},
	{
		...
		ids: {
			"route": "route:/api/admin/leads/:id",
			"page": null,
			"routeHandler": "route-handler:/api/admin/leads/:id",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
	...
	},
	{
		...
		ids: {
			"route": "route:/api/admin/patients/:id",
			"page": null,
			"routeHandler": "route-handler:/api/admin/patients/:id",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
		},
		pattern: "/api/admin/patients/:id",
		patternParts: [
			"api",
			"admin",
			"patients",
	...
	},
	{
		...
		ids: {
			"route": "route:/api/admin/professionals/:id",
			"page": null,
			"routeHandler": "route-handler:/api/admin/professionals/:id",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
		},
		pattern: "/api/admin/professionals/:id",
		patternParts: [
			"api",
			"admin",
			"professionals",
	...
	},
	{
		...
		ids: {
			"route": "route:/admin/cuidadores",
			"page": "page:/admin/cuidadores",
			"routeHandler": null,
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/", "layout:/admin"],
			"templates": [],
		...
		pattern: "/admin/cuidadores",
		patternParts: ["admin", "cuidadores"],
		isDynamic: false,
		params: [],
		rootParamNames: [],
		...
		routeHandler: null,
		layouts: [layout_exports$3, layout_exports$1],
		routeSegments: ["admin", "cuidadores"],
		templateTreePositions: [],
		layoutTreePositions: [0, 1],
		templates: [],
		errors: [null, null],
	...
	},
	{
		...
		ids: {
			"route": "route:/admin/familias",
			"page": "page:/admin/familias",
			"routeHandler": null,
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/", "layout:/admin"],
			"templates": [],
	...
	},
	{
		...
		ids: {
			"route": "route:/admin/leads",
			"page": "page:/admin/leads",
			"routeHandler": null,
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/", "layout:/admin"],
			"templates": [],
	...
	},
	{
		...
		ids: {
			"route": "route:/admin/pacientes",
			"page": "page:/admin/pacientes",
			"routeHandler": null,
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/", "layout:/admin"],
			"templates": [],
	...
	},
	{
		...
		ids: {
			"route": "route:/admin/pacientes/novo",
			"page": "page:/admin/pacientes/novo",
			"routeHandler": null,
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/", "layout:/admin"],
			"templates": [],
	...
	},
	{
		...
		ids: {
			"route": "route:/admin/profissionais",
			"page": "page:/admin/profissionais",
			"routeHandler": null,
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/", "layout:/admin"],
			"templates": [],
		...
		pattern: "/admin/profissionais",
		patternParts: ["admin", "profissionais"],
		isDynamic: false,
		params: [],
		rootParamNames: [],
		...
		routeHandler: null,
		layouts: [layout_exports$3, layout_exports$1],
		routeSegments: ["admin", "profissionais"],
		templateTreePositions: [],
		layoutTreePositions: [0, 1],
		templates: [],
		errors: [null, null],
	...
	},
	{
		...
		ids: {
			"route": "route:/api/admin/assignments",
			"page": null,
			"routeHandler": "route-handler:/api/admin/assignments",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
		},
		pattern: "/api/admin/assignments",
		patternParts: [
			"api",
			"admin",
			"assignments"
	...
	},
	{
		...
		ids: {
			"route": "route:/api/admin/caregivers",
			"page": null,
			"routeHandler": "route-handler:/api/admin/caregivers",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
		},
		pattern: "/api/admin/caregivers",
		patternParts: [
			"api",
			"admin",
			"caregivers"
	...
	},
	{
		...
		ids: {
			"route": "route:/api/admin/contracts",
			"page": null,
			"routeHandler": "route-handler:/api/admin/contracts",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
	...
	},
	{
		...
		ids: {
			"route": "route:/api/admin/patients",
			"page": null,
			"routeHandler": "route-handler:/api/admin/patients",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
		},
		pattern: "/api/admin/patients",
		patternParts: [
			"api",
			"admin",
			"patients"
	...
	},
	{
		...
		ids: {
			"route": "route:/api/auth/forgot-password",
			"page": null,
			"routeHandler": "route-handler:/api/auth/forgot-password",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
	...
	},
	{
		...
		ids: {
			"route": "route:/api/auth/reset-password",
			"page": null,
			"routeHandler": "route-handler:/api/auth/reset-password",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
	...
	},
	{
		...
		ids: {
			"route": "route:/api/professionals",
			"page": null,
			"routeHandler": "route-handler:/api/professionals",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
		...
		patternParts: ["api", "professionals"],
		isDynamic: false,
		params: [],
		rootParamNames: [],
		page: null,
	...
	},
	{
		...
		ids: {
			"route": "route:/api/records",
			"page": null,
			"routeHandler": "route-handler:/api/records",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
	...
	},
	{
		...
		ids: {
			"route": "route:/api/solicitacoes",
			"page": null,
			"routeHandler": "route-handler:/api/solicitacoes",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
		},
		pattern: "/api/solicitacoes",
		patternParts: ["api", "solicitacoes"],
		isDynamic: false,
		params: [],
		rootParamNames: [],
		page: null,
	...
	},
	{
		...
		ids: {
			...
			"routeHandler": null,
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/", "layout:/familia"],
			"templates": [],
			"slots": {}
		},
		pattern: "/familia",
		patternParts: ["familia"],
		isDynamic: false,
		params: [],
		rootParamNames: [],
		page: page_exports$8,
		routeHandler: null,
		layouts: [layout_exports$3, layout_exports],
		routeSegments: ["familia"],
		templateTreePositions: [],
		layoutTreePositions: [0, 1],
		templates: [],
		errors: [null, null],
	...
	},
	{
		...
		pattern: "/solicitar-cuidado",
		patternParts: ["solicitar-cuidado"],
		isDynamic: false,
		params: [],
		rootParamNames: [],
		page: page_exports$4,
		routeHandler: null,
		layouts: [layout_exports$3],
		routeSegments: ["solicitar-cuidado"],
		templateTreePositions: [],
		layoutTreePositions: [0],
		templates: [],
		errors: [null],
	...
	},
	{
		...
		routeHandler: null,
		layouts: [layout_exports$3],
		routeSegments: ["trabalhe-conosco"],
		templateTreePositions: [],
		layoutTreePositions: [0],
		templates: [],
		errors: [null],
	...
	},
	{
		...
		ids: {
			...
			"page": null,
			"routeHandler": "route-handler:/api/contracts/:id",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
		},
		pattern: "/api/contracts/:id",
		patternParts: [
			"api",
			"contracts",
			":id"
...
var __publicFiles = new Set([
	"/file.svg",
	"/globe.svg",
	"/images/vellora-contact-background.webp",
	"/images/vellora-hero-background.webp",
	"/next.svg",
	"/vercel.svg",
	"/window.svg"
]);
...
var _virtual_vinext_rsc_entry_default = createAppRscHandler({
	...
	setNavigationContext: setAppNavigationContext,
	staticParamsMap: {
		"/cuidador/paciente/:id/historico": null,
		"/cuidador/paciente/:id/registro": null,
		"/admin/pacientes/:id": null,
		"/familia/paciente/:id": null
	},
...
var worker_entry_default = { async fetch(request, env, ctx) {
	...
	}, [...DEFAULT_DEVICE_SIZES, ...DEFAULT_IMAGE_SIZES]), request);
	return withSecurityHeaders(await app_router_entry_default.fetch(request, env, ctx), request);
} };
//#endregion
