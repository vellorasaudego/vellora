...
function configuredAppUrl(): string {
  ...
  if (url.protocol !== "https:" && !(local && url.protocol === "http:")) {
    throw new Error("VELLORA_APP_URL deve usar HTTPS.");
  }
  return url.origin;
}

export function isPasswordEmailConfigured(): boolean {
  if (!runtimeValue("RESEND_API_KEY") || !runtimeValue("VELLORA_EMAIL_FROM")?.trim()) {
    return false;
  }
  try {
    configuredAppUrl();
...
export async function sendPasswordResetEmail(input: {
  ...
  const response = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      ...
      text:
        `Recebemos uma solicitação para redefinir sua senha na Vellora Saúde.\n\n` +
  ...
  if (!response.ok) {
    throw new Error(`Falha no envio do e-mail (${response.status}).`);
  }
}
